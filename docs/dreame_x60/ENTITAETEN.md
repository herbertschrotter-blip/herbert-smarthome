...
|---|---|---|---|---|---|---|
| `button.heidi_clear_warning` | Knopf | Clear Warning | Quittiert die anstehende Warnung (nur dann verfügbar; Liste unten „Warnungen und Fehler“) | diagnostic |  | geplant Modul R: ✕ am gelben Hinweis-Chip des Roboter-Panels (Entscheidung Herbert 17.09., Mockup `warnung.html`, Abnahme offen) |
...
| `sensor.heidi_mapping_time` | Sensor | Mapping Time | Dauer der laufenden Kartenerstellung |  |  | – |
| `sensor.heidi_relocation_status` | Sensor | Relocation Status | Sucht Heidi gerade ihre Position auf der Karte neu |  |  | – |
| `sensor.heidi_state` | Sensor | State | Grober Betriebszustand (cleaning, docked, paused …), wie `vacuum.heidi` |  |  | – |
| `sensor.heidi_status` | Sensor | Status | Genauer Status (Charging, Sweeping, Washing, Drying …) |  |  | Roboter-Panel (`dx-hero`) Statustext |
| `sensor.heidi_stream_status` | Sensor | Stream Status | Zustand des Kamera-Livestreams |  |  | – |
...
| `sensor.heidi_task_type` | Sensor | Task Type | Auftragsart (ganze Wohnung, Räume, Zone, Punkt) |  |  | – |
| `switch.heidi_resume_cleaning` | Schalter | Resume Cleaning | Unterbrochenen Lauf nach dem Laden fortsetzen | config |  | – |
