...
| F.1 | **Diagnose-Protokoll** (Modul F, DX-068: HA-Zustände mit Auslöser, Debuglog der Integration, Kartenanzeige) | gebaut – **alle drei Schichten eingespielt 18.09.** (Backend: `diag.py`, zwei Automationen, `logger`, `tools/diag.js`, 12 Tests auf dem Pi grün; Karte: Schicht 3 PD-017, Version 2.0.0-alpha.32); Probe 18.09.: App-Start 20:39 = „extern“, Dashboard-Start 20:48 = „Benutzer Herbert Schrotter“, Kartenanzeige live mitprotokolliert; offen: Probe Planerstart (nächster Planerlauf), dann `tracker done` |
| F.2 | **Seite „Dev“, Tickets, Knopf „Fehler melden“** (Modul F, DX-069: Live-Zeitleiste, Auswertung mit 23 Regeln, Ticketsystem HT-NNNN mit Status, Skill `ticket`; Teilschritte F.2a Backend, F.2b Karte, F.2c Skill) | Mockup `dev.html` abgenommen 18.09. (PD-018 freigegeben); **F.2a Backend fertig und eingespielt 18.09.** (Auswertung 23 Regeln, Tickets HT-NNNN, 45 Python-Tests); **F.2b Karte fertig und eingespielt 18.09. (Version 2.0.0-alpha.33)** – Seite Dev, Ticket-Ansicht, „Fehler melden“, erweiterte Anzeige-Meldung mit Lebenszeichen; Sichtprüfung Herbert offen; **F.2c Skill `ticket` fertig 18.09.** (Skills-Repo v0.36.0, Ticket-Profil in der CLAUDE.md, `tools/ticket.ps1`); offen: Skill bei claude.ai hochladen (Herbert), Live-Trigger-Tests, Probe an HT-0001 |
...
| **F Backend** (parallel) | **F.1 Diagnose-Protokoll (DX-068) – alle drei Schichten eingespielt 18.09. (alpha.32), Probe Planerstart offen**, Planer nachholen (DX-048), 0.4-Entscheidung (DX-006), 2.7 **[PC]** (DX-019) | Paket, Automationen, Prognose | offen |
...
- S HA-Ereignis `dreame_x60_meldung` (F.2, PD-018, Backend seit 18.09., Karte ab F.2b): `{text, stichworte[], seite, version, client, werte{…}}` → Zeile `art: meldung` → Ticket `HT-NNNN`; nur Admin-Benutzer
- L/S Dienste mit Antwort (`return_response`, `args` = base64 eines JSON-Objekts): `shell_command.heidi_diag_tail` `{n, vor}` → `{zeilen[], aelter}`; `shell_command.heidi_ticket` `{cmd: liste|zeige|text|status|notiz|verwerfen, nr, status, dx, commit, version, grund, text, wer}` → `{ok, ticket|tickets, zaehler, text, fehler}` (einziger Schreibweg der Tickets); L `shell_command.heidi_diag_auswertung` → `{funde, info[], tickets, zaehler, starts[]}`. Ab F.2b meldet `dreame_x60_anzeige` zusätzlich `modus_ha, saug_ha, wasser_ha, jetzt, reihenfolge[], knoepfe[], laden, station, schaetzung_min` sowie `puls` (Lebenszeichen alle 30 s) und `ende` (Fenster geschlossen)
- S HA-Ereignis `dreame_x60_anzeige` (Diagnose-Protokoll Schicht 3, F.1, PD-017, 18.09.): `POST events/dreame_x60_anzeige` mit `{seite, version, client, geaendert[], werte{kopf, schritt, hinweis, akku, fortschritt, zustand}}`; nur mit Kartenoption `diagnose: true` und nur bei Admin-Benutzern (`hass.user.is_admin`); `contract.ts`: `HA_EVENTS.anzeige`, `eventPath()`; Empfänger: Automation `heidi_diagnose_protokoll`
...
| `dev` | **Nur für Admin-Benutzer (F.2, PD-018).** Kacheln „Protokoll“, „Starts heute“, „Tickets“ (Zähler), Ticketliste mit Filter und Regelübersicht, Zeitleiste (Filter, Suche, Live); Ticket-Ansicht als Dialog; Eintrag „Dev“ in Seitenleiste, Symbolleiste und Tab-Leiste (sechster Platz). Knopf „Fehler melden“ in der Kopfzeile **jeder** Seite links neben der Uhr. **Abgenommen von Herbert am 18.09.2026 (Mockup `dreame_x60/mockups/dev.html`)** |
...
- Umsetzung Schicht 3 (18.09., PD-017, Version 2.0.0-alpha.32): `src/domain/anzeige.ts` (Schnappschuss + Diff, rein), `DxApi.fireEvent` (nur Admin, wirft nie), Shell meldet in `updated()` – höchstens einmal je Sekunde, nur bei geänderter Anzeige, nur mit `diagnose: true` (im Dashboard für alle sechs Ansichten gesetzt); Automation schreibt `art: anzeige`. Tests: `anzeige.test.ts`, `api.test.ts` (fireEvent), `hero.js` (8 Prüfungen „Diagnose“), `test_diag.py` (Anzeige-Zeile).

**F.2 Seite „Dev“ + Knopf „Fehler melden“** (ClickUp DX-069, Wunsch Herbert 18.09.2026; Kartenteil von Modul F)
- Voraussetzung: F.1; **Mockup `dreame_x60/mockups/dev.html` abgenommen** und PD-018 freigegeben (Mockup-Pflicht, Regel 2)
- Ziel: Das Diagnose-Protokoll bekommt eine Oberfläche. (1) **Seite „Dev“** – siebte Seite der Karte, nur für Admin-Benutzer (`hass.user.is_admin`): Kachel „Protokoll“ (läuft, Zeilen/Größe heute, Schichten, Aufbewahrung), Kachel „Starts heute“ mit Quelle (wie `tools/diag.js`), Kachel „Meine Meldungen“, Kachel „Zeitleiste“ (neueste oben, Filter Alles/Roboter/Dienste/Automationen/Anzeige/Meldungen, Suche, „Live“, Zeile antippen = Rohzeile, „Ältere laden“). Daten über einen Dienstaufruf mit Antwort (`shell_command.heidi_diag_tail` → `diag.py tail <n> [vor <ts>]`, JSON) – kein neuer Helfer, keine Datei unter `/local`. Eintrag „Dev“ in Seitenleiste, Symbolleiste **und Tab-Leiste am Handy** (sechster Platz, nur für Admin; alle anderen sehen weiter fünf – Herbert, 18.09.). (2) **Knopf „Fehler melden“** in der Kopfzeile jeder Seite, **links neben der Uhr** (nach den Einrichtungs-Symbolen PD-014; ≤ 760 px ist die Uhr ausgeblendet, dort steht der Knopf als Symbol an ihrer Stelle; nur Admin): Dialog `dx-dialog` (modal, schmal Sheet) mit Schnellwahl-Chips und Textfeld → HA-Ereignis `dreame_x60_meldung` {text, stichworte, seite, version, client, anzeige-Schnappschuss, Roboter-Zustand} → Zeile `art: meldung`; Toast mit Uhrzeit; auf der Seite Dev gelb hervorgehoben; `tools/diag.js` listet Meldungen. (2b) **Anzeige-Umfang von Schicht 3 erweitern** (Ergänzung zu PD-017): `anzeigeSnapshot` meldet zusätzlich die drei Werte-Knöpfe mit Quelle (`select` oder Kartendaten, PD-010), den Streifen (jetzt, danach) und die Auftrag-Kachel (Reihenfolge, nächster Raum, Räume x/n) – sonst kann die Auswertung diese Felder nicht prüfen. (3) **Auswertung „Auffälligkeiten“ + „Prompt für Claude“** (Herbert, 18.09.): `diag.py auswertung [tag]` sucht selbst nach Fehlern – Regeln: Anzeige ≠ Roboter länger als 10 s (Zeilen `anzeige` gegen `zustand`), **Werte ≠ Roboter** (angezeigte drei Werte Modus/Saugstufe/Wasser gegen die im Lauf gültigen: Raum-Werte des aktuellen Raums, wenn `customized_cleaning` an ist, sonst die globalen Attribute `cleaning_mode`/`suction_level`/`mop_pad_humidity` von `vacuum.*` – ein Start in der App schaltet „Angepasste Reinigung“ aus), **Reihenfolge ≠ gefahren** (angekündigte Reihenfolge/nächster Raum aus Streifen und Auftrag-Kachel gegen die Folge der `current_segment`-Wechsel innerhalb `active_segments`; Durchfahren zählt nicht; typischer Fall: Reihenfolge-Helfer noch vom letzten HA-Start, Raumauswahl in der App), Dienstaufruf ohne Zustandswechsel in 30 s, Fehler/Traceback der Integration (Debuglog, gleiche Meldung gezählt), Roboter-Entität länger als 10 min `unavailable`, Start ohne HA-Auslöser, Warnung/Fehler des Roboters, Lücken-Marke im Protokoll; Schwellen als benannte Konstanten. **Vollständige Regelliste (Herbert, 18.09.: „alle aufnehmen“), 23 Regeln in drei Gruppen.** *Anzeige gegen Roboter* (braucht die Meldungen der Karte): A1 Kopf/Arbeitsschritt ≠ Roboter > 10 s · A2 Werte-Knöpfe ≠ gültige Werte · A3 Reihenfolge ≠ gefahren · A4 Raum-Chip/„Jetzt“ ≠ `current_segment` · A5 Knöpfe passen nicht zum Zustand (z. B. „Start“ sichtbar, Roboter `cleaning`) · A6 Akku oder Lade-Blitz ≠ Roboter · A7 Fortschritt außerhalb eines Laufs · A8 Station: Karte „Bereit“, während `washing`/`drying`/Absaugen läuft (wirksam ab Modul C, 4.9) · A9 Warnung des Roboters ohne Hinweis-Chip · A10 Karte bekommt keine Daten (Roboter ändert sich, Fenster meldet länger als 60 s nichts, obwohl es kurz davor aktiv war). *Bedienung und Planer* (nur aus dem Protokoll): B1 Dienstaufruf ohne Zustandswechsel in 30 s · B2 gewählte Räume (`vacuum_clean_segment.segments`) ≠ `active_segments` · B3 `sensor.heidi_automatik_status` sagt „startet“, aber kein Start in 2 min · B4 Automatik-Lauf fertig, `heidi_letzte_auto_reinigung` nicht gesetzt · B5 Planerstart außerhalb der Uhrzeit des Eintrags oder obwohl eine „stört“-Person zu Hause ist · B6 `input_text.heidi_raum_snapshot` 10 min nach dem Andocken nicht leer · B7 `switch.*_customized_cleaning` 10 min nach dem Andocken noch aus · B8 geschätzte Dauer weicht > 30 % von der echten ab (wirksam ab 4.8, wenn die Karte die Schätzung meldet). *Roboter und Technik*: T1 Start ohne HA-Auslöser (Info) · T2 Warnung/Fehler des Roboters · T3 Roboter oder Roboter-Entität > 10 min `unavailable` · T4 Fehler/Traceback der Integration (gleiche Meldung gezählt) · T5 Lücken-Marke im Protokoll. Dafür meldet die Karte zusätzlich (Anzeige-Umfang): sichtbare Knöpfe, Raum-Chip, Lade-Blitz, Hinweis-Chip, Stationszeile und – sobald gebaut – Schätzung und Planer-Status. Jede Regel ist eine reine Funktion in `diag.py` mit benannter Schwelle, eigenem Testfall und Gegenprobe; Regeln für noch nicht gebaute Bausteine bleiben still, solange die Karte das Feld nicht meldet. Nicht automatisch prüfbar: was die Dreame-App selbst anzeigt → Knopf „Fehler melden“. Ergebnis je Fund: Art, Schwere (Fehler/Hinweis/Info), Text, Zeitraum, Anzahl, Bezugszeilen. Kachel „Auffälligkeiten“ auf der Seite Dev (Antippen springt zur Stelle). Aus jedem Fund, jeder Meldung und dem ganzen Tag („Tagesbericht“) entsteht ein **fertiger Prompt**: Projekt/Branch/Versionen, Pflicht-Docs, Befund, Auszug der Zeitleiste (±60 s), Rohmeldungen der Integration im selben Fenster, „Wo suchen“ (Zuordnung Regel/Entität → Dateien, gepflegt in `diag.py`), Aufgabe; Dialog mit „Kopieren“ (Zwischenablage), derselbe Text über `node tools\diag.js --prompt`.
- **Ticketsystem (Herbert, 18.09.: statt eines losen Prompts):** Jede Meldung („Fehler melden“) und jeder Fund der Auswertung ab Schwere „Hinweis“ wird ein **Ticket** mit fortlaufender Nummer `HT-0001` … (Heidi-Ticket; Info-Funde wie „Start per App“ erzeugen keines). **Ein Ticket je Problem, nicht je Vorkommen:** Schlüssel = Regel + Signatur (z. B. T4 + Fehlermeldung, A2 + Feld); Wiederholungen zählen hoch (`anzahl`, `zuletzt`); tritt ein gelöstes Ticket wieder auf, wird es mit Vermerk „wieder aufgetreten“ erneut geöffnet. **Beweise werden beim Anlegen gesichert** (Zeitleisten-Auszug ±60 s, Rohmeldungen der Integration im selben Fenster, „Wo suchen“, Versionen), weil die Protokolle nach 30/14 Tagen gelöscht werden; Tickets werden nie automatisch gelöscht. Ablage `/config/prognose/diag/tickets.json` (nie ins Repo); **einziger Schreibweg** ist `diag.py ticket …` über `shell_command.heidi_ticket` (Karte und Claude Code gleich: `liste`, `zeige <nr>`, `status <nr> <status> [--dx --commit --version]`, `notiz <nr> <text>`, `verwerfen <nr> <grund>`). **Status:** `neu` → `angenommen` (ClickUp DX-… verknüpft) → `in_arbeit` → `geloest` (Commit, Version) → `geschlossen` (Herbert hat geprüft); daneben `verworfen` (kein Fehler, mit Grund). **ClickUp bleibt der Ort der Arbeit:** eine DX-Aufgabe entsteht erst, wenn ein Ticket gemeinsam angegangen wird, und nur über den tracker-Skill; das Ticket merkt sich DX-Nummer, Commit, Version – keine doppelte Pflege, nicht jedes Ticket wird eine Aufgabe. **Skill `ticket`** (projektneutral, „Ticket-Profil“ in der CLAUDE.md: Präfix HT, Dienst, Status-Werte): `ticket liste`, `ticket HT-0007` → Claude holt das Ticket selbst vom Pi (kein Kopieren nötig) und arbeitet immer gleich: 1 Befund in einfachen Worten · 2 Auswahlfrage „echter Fehler?“ (sonst verwerfen) · 3 Ursache suchen · 4 `tracker neu/start` · 5 Fix mit Test (code-erstellen) · 6 einspielen · 7 Ticket `geloest` + `tracker done` · 8 Herbert prüft → `geschlossen`. **Immer nur ein Ticket, jeder Übergang eine Auswahlfrage, nichts automatisch.** Oberfläche: Kachel „Tickets“ (Zähler neu / in Arbeit / gelöst, Ablauf), Ticketliste mit Filter Offen/Gelöst/Alle, Ticket-Ansicht mit den gesicherten Beweisen und den Knöpfen „Verwerfen – kein Fehler“, „Notiz“, „Kopieren“ (Notweg), „Schließen“; nach „Fehler melden“ nennt der Toast die Ticketnummer.
- **Teilschritte (je ein Commit, je eine ClickUp-Unteraufgabe von DX-069):** **F.2a Backend** – `diag.py`: `tail`, `auswertung` (23 Regeln), `ticket`-Befehle, Tests; `shell_command`s; keine Kartenänderung. **F.2b Karte** – Anzeige-Umfang erweitern, Seite Dev (Protokoll, Starts, Tickets, Zeitleiste), „Fehler melden“, `DxApi`-Methoden, E2E `dev.js`. **F.2c Skill `ticket`** – im Skills-Repo (Pfad auf diesem PC vorher klären), Ticket-Profil in der CLAUDE.md, Probe an einem echten Ticket.
- Nicht ändern: Zeilenformat v1 des Protokolls (nur neue Art `meldung`), `runlog.csv`, bestehende Seiten (Tab-Leiste: nur der zusätzliche Platz „Dev“ für Admin); Schreiben nur über `DxApi`; keine externen Ressourcen (Regel 14); Render-Ruhe der übrigen Seiten (Regel 10).
- Akzeptanz: Seite Dev zeigt die letzten Zeilen, neue erscheinen ohne Neuladen (Live an), Filter und Suche wirken; jede Regel der Auswertung findet ihren Testfall und bleibt bei unauffälligen Daten still; der Prompt enthält Befund, Zeitleisten-Auszug, Rohmeldungen und „Wo suchen“ und landet per „Kopieren“ in der Zwischenablage; „Fehler melden“ schreibt genau eine Zeile `art: meldung` mit Text, Benutzer, Seite, Version, Anzeige-Schnappschuss; Nicht-Admin: kein Eintrag Dev, kein Knopf, kein Fehler; leere Meldung wird nicht gesendet; Fehler beim Senden → Toast, Dialog bleibt offen (Regel 20).
- Tests: `dev.js` (E2E: Zeitleiste aus Stub-Antwort, Filter, Live-Abruf, Melden → ein Ereignis, Nicht-Admin), `api.test.ts` (diagTail, reportProblem), `test_diag.py` (tail, Art meldung, je Regel der Auswertung ein Fall + Gegenprobe, Prompt-Text), `npm test`, `npm run lint`
- Dateien: `dreame_x60/card/src/components/dx-dev.ts`, `dx-report-dialog.ts`, `src/pages.ts`, `src/dreame-x60-panel.ts`, `src/ha/api.ts`, `src/ha/contract.ts`, `src/i18n/de.ts`, `ha/prognose/diag.py`, `ha/packages/heidi.yaml`, `ha/automations.yaml`, `ha/dashboards/dreame_x60.yaml`, `tools/diag.js`
...
- [2026-09-18] [F.2c] Befund · Info (**Skill `ticket` angelegt**, Skills-Repo `claude-skills-bpm` v0.36.0, Commit 4c498ce): projektneutral, acht feste Schritte (Ticket holen und einfach erklären · echter Fehler? · Ursache belegen · Aufgabe über tracker · Fix mit Test über code-erstellen · einspielen · Ticket gelöst + tracker done + Doku · Prüfung durch Herbert → geschlossen), Kernregeln: ein Ticket zur Zeit, jeder Übergang eine Auswahlfrage, Ticket mit Beweisen ist die Quelle, einziger Schreibweg, Schließen macht Herbert; Kommandos `ticket liste|<Nr>|notiz|verwerfen|schließen|auswertung`. Projektwerte im neuen **Ticket-Profil** der CLAUDE.md; Werkzeug `tools/ticket.ps1` als Hülle um `shell_command.heidi_ticket` (am echten HA geprüft: liste, zeige, status mit Pflichtangaben, notiz, verwerfen, auswertung). **Befund Skills-Repo-Pfad:** OneDrive liegt auf diesem PC unter `D:\OneDrive` (Benutzer-Umgebungsvariable `OneDrive`) – der Pfad aus dem Tracker-Profil stimmt, in Teil 5 wurde unter `C:` gesucht; Tracker-Profil ergänzt. **Erster Durchlauf von Hand – HT-0004** „Knöpfe passen nicht zum Zustand docked“ (Regel A5, 2×, entstanden direkt nach dem Einspielen von alpha.33): Ursache nicht die Karte, sondern die Auswertung – die Automation übergibt Listen als Text (`"['start', 'locate']"`), Regel A5 verglich zeichenweise; behoben in `diag_regeln.py` (`texte()` liest beide Formen), Test mit Gegenprobe, Commit 94be28f, Ticket mit Notiz auf `geloest`; Schließen durch Herbert offen. Abweichung vom künftigen Ablauf: Claude hat diesen Fall ohne Auswahlfragen behoben, weil es ein eigener Fehler mitten im Bau war – ab jetzt gilt der Skill. Offen: Herbert lädt `skills/ticket/SKILL.md` bei claude.ai hoch; Live-Trigger-Tests (`test-prompts.md`) in neuer Sitzung; erste echte Probe mit HT-0001 (Integrationsfehler „Map render Failed“).
- [2026-09-18] [F.2b] Befund · Info (**Karte gebaut und eingespielt, Version 2.0.0-alpha.33**, Ressource per `ha-ws.js`, Dashboard mit siebter Ansicht `dev`, kein Neustart): `dx-dev` (Seite Dev: Kacheln Protokoll, Starts heute, Tickets-Zähler, Ticketliste mit Filter Offen/Gelöst/Alle und aufklappbarer Regelübersicht, Zeitleiste mit Filter, Suche, „Live“ alle 10 s, Rohzeile beim Antippen, „Ältere laden“ ohne Dubletten; Sprung von Start/Ticket zur Stelle), `dx-ticket` (Ticket-Ansicht mit dem gesicherten Text, Notiz, „Verwerfen – kein Fehler“ nur mit Grund, „Kopieren“), `dx-report` (Schnellwahl + Text, leere Meldung wird nicht gesendet, Fehler lassen den Dialog offen, Toast nennt die Ticketnummer nach 1,5 s Nachschlagen). Shell: Knopf „Fehler melden“ links neben der Uhr auf jeder Seite (nach den Einrichtungs-Symbolen; ≤ 760 px nur Symbol), Overlays `report`/`ticket`, Seite `dev` nur mit `hass.user.is_admin` – sonst zeigt die Karte die Übersicht; `dx-nav` mit Eigenschaft `admin`, Eintrag „Dev“ in Seitenleiste, Symbolleiste und Tab-Leiste (bis zu sieben Plätze, Beschriftung mit Auslassung). **Anzeige-Meldung erweitert** (`domain/anzeige.ts`): `knoepfe`, `laden`, im Lauf `modus_ha`/`saug_ha`/`wasser_ha` (Anzeigetext → HA-Wert über `RV_HA`), `jetzt`, `reihenfolge`; Felder ohne Aussage fehlen im Ereignis (Regel bleibt still); **Lebenszeichen alle 30 s** (`puls`, alle Werte) und `ende` beim Entfernen der Karte bzw. `pagehide`. `DxApi`: `diagTail`, `diagStatus`, `tickets`, `ticket`, `ticketVerwerfen`, `ticketNotiz`, `reportProblem`; Dienste mit Antwort über `callWS call_service return_response` (am echten HA geprüft), Argumente als UTF-8-base64. Vertrag: `HA_EVENTS.meldung`, `SERVICES.diagTail|diagStatus|ticket`; 95 neue Texte in `de.ts` (`dev.*`, `report.*`, `ticket.*`, `api.*`). **Tests:** `dev.js` neu (29 Prüfungen: Kacheln, Starts mit Quelle, Filter, Suche, Rohzeile, Ticket verwerfen nur mit Grund, Melden → genau ein Ereignis mit Kontext, `ende`, 390 px ohne Überlauf, ohne Admin-Recht nichts), `diag.test.ts` neu (4), `anzeige.test.ts` neu gefasst (4), hero.js unverändert 42 (Render-Ruhe bleibt); gesamt 100 Unit, 10 E2E, Lint grün. **Probe am echten HA:** Ereignis `dreame_x60_meldung` → Zeile `art: meldung` → Ticket HT-0003 (als Probe verworfen); die Geräte-Erfassung nimmt jetzt auch `device_tracker.dreame_vacuum_r6001a` mit (Entität ohne „heidi“ im Namen). Abweichung vom Mockup: Hauptknopf grün wie überall in der Karte (Mockup blau), Tab-Leiste mit aktiver Prognose sieben statt sechs Plätze. Offen: Sichtprüfung Herbert (Strg+F5), F.2c Skill `ticket`.
- [2026-09-18] [F.2a] Befund · Info (**Backend gebaut und eingespielt**, ohne Kartenänderung, Version bleibt 2.0.0-alpha.32, ohne Neustart): `ha/prognose/diag_regeln.py` – `auswerten()` spielt das Protokoll durch (Welt = letzter Zustand und Attribute je Entität, dazu der letzte **gültige** Zustand, weil Raum-Selects und „Angepasste Reinigung“ im Lauf `unavailable` sind) und prüft die 23 Regeln; Schwellen als benannte Konstanten (`TOL_S` 10, `DIENST_WIRKUNG_S` 30, `START_FENSTER_S` 90, `PLANER_START_S` 120, `NACH_LAUF_S` 600, `WEG_S` 600, `PULS_MAX_S` 90, `SCHAETZUNG_PCT` 30). **Entscheidung zur Anzeige-Prüfung:** geprüft wird, wenn die Karte meldet; ein Treffer wartet 10 s und verfällt, wenn die Karte sich korrigiert oder der Roboter nachzieht – ohne neue Meldung entsteht nie ein Fund, ein geschlossenes Browserfenster erzeugt also keine falschen Funde. Dafür braucht F.2b ein Lebenszeichen der Karte (`puls` alle 30 s mit allen Werten, `ende` beim Schließen), sonst bleiben stehende Anzeigen unentdeckt (A10). Felder, die die Karte noch nicht meldet, lassen ihre Regel still. `ha/prognose/diag_tickets.py` – Tickets `HT-NNNN`: ein Ticket je Signatur, je Tag zählt die höchste gesehene Anzahl (wiederholtes Auswerten zählt nicht doppelt), Wiederöffnen nur bei Vorkommen nach „gelöst“, verworfene bleiben verworfen, Pflichtangaben (angenommen → DX, gelöst → Commit, verworfen → Grund), `als_text()` = Tickettext für Claude. `diag.py`: `tail`, `auswertung`, `ticket` (Sperrdatei + atomares Schreiben, Fehler als JSON), Meldung → Ticket mit gesichertem Auszug (±60 s, bei über Stunden gezählten Funden um das letzte Vorkommen), Log-Kopf erkennt Thread-Namen mit Klammern. Automation `heidi_diagnose_auswertung` alle 10 min; `heidi_diagnose_protokoll` hört zusätzlich auf `dreame_x60_meldung`. `tools/diag.js --tickets [alle]`. **Tests:** `test_diag_regeln.py` (25: je Regel Fall + Gegenprobe, geschlossenes Fenster, unauffälliger Tag), `test_diag_tickets.py` (8), `test_diag.py` (12) = 45, lokal (Python 3.14) und auf dem Pi grün; `check_config` valid. **Probe mit den echten Daten vom 18.09.:** HT-0001 „Map render Failed … 'Segment' object has no attribute 'area'“ (104×, Regel T4), HT-0002 „Akku 99 % gegen 100 %“ = Claudes Test-Ereignis → über den Dienst verworfen (Probe des Schreibwegs); Start per App nur als Info (kein Ticket). Korrektur zur Notiz von 20:48: der „Fortschritt 0 %“ nach dem Kurzlauf stand nur 9 s (Sensor wird 9 s nach dem Andocken `unavailable`) – unter der 10-s-Schwelle, zu Recht kein Fund. Offen: F.2b Karte, F.2c Skill.
- [2026-09-18] [F.2] Befund · Info (**Mockup `dreame_x60/mockups/dev.html` abgenommen von Herbert am 18.09.2026**, Auswahlfrage „Abgenommen, F.2a gleich bauen“): Umfang der Abnahme – Seite Dev (nur Admin; Seitenleiste, Symbolleiste, Tab-Leiste am Handy als sechster Platz) mit Kacheln Protokoll, Starts heute, Tickets, Ticketliste (Filter Offen/Gelöst/Alle), Regelübersicht (23 Regeln), Zeitleiste (Filter, Suche, Live, Ältere laden); Ticket-Ansicht mit gesicherten Beweisen und „Verwerfen – kein Fehler“, „Notiz“, „Kopieren“, „Schließen“; Knopf „Fehler melden“ auf jeder Seite links neben der Uhr (≤ 760 px Symbol an Stelle der Uhr), Dialog mit Schnellwahl und Text, Toast mit Ticketnummer; Zustand ohne Admin-Recht. Bewusst weggelassen: Tagesauswahl (nur „heute“), Bildschirmfoto an der Meldung, direktes Anlegen von ClickUp-Aufgaben aus der Karte (ClickUp nur über den Skill `ticket`/tracker). PD-018 freigegeben. Reihenfolge: F.2a Backend → F.2b Karte → F.2c Skill. Entscheidung Herbert: abgenommen (18.09.)
- [2026-09-18] [F.2] Wunsch · Functional (Herbert nach der Probe: „gibt es keine UI und ein neues Tab Dev? und den Button um Fehler zu melden?“): War nicht Teil von F.1/DX-068 (dort nur Protokoll, Kartenmeldung, Auswertung per Skript). Neue Aufgabe **F.2 / DX-069**, Karte in Abschnitt 8, PD-018 offen, Mockup `dreame_x60/mockups/dev.html` (Desktop 1180 px + 390 px; Zustände: Seite Dev, Dialog offen, nach dem Senden, Benutzer ohne Admin-Recht; Token-Abgleich mit `tokens.ts` ohne Abweichung). Entscheidungen im Mockup: Dev nur für Admin; **nach Herberts Rückmeldung (18.09.): Dev auch am Handy in der Tab-Leiste (sechster Platz), Knopf „Fehler melden“ auf jeder Seite links neben der Uhr** (erste Fassung hatte Dev am Handy unter Einstellungen); neuer Zustand im Mockup „Knopf auf jeder Seite (Beispiel Übersicht)“; **dritte Rückmeldung Herbert (18.09., mit Bildschirmfoto der Werte-Knöpfe): „wird es auch gemerkt, wenn der Roboter läuft und die Knöpfe Modus was anderes anzeigen, oder wenn die Reihenfolge der Räume nicht passt, weil ich in der App Räume gewählt habe?“** – heute nein (Schicht 3 meldet diese Felder nicht, Regeln fehlen); am echten System geprüft: die Wahrheit steht im Protokoll (`cleaning_mode`, `suction_level`, `customized_cleaning`, `current_segment`, `active_segments`), Live-Beispiele: Karte „Standard“ gegen global `Quiet`, Reihenfolge-Helfer „4“ gegen App-Auftrag [5] → Umfang F.2 um Anzeige-Felder und die Regeln „Werte ≠ Roboter“ und „Reihenfolge ≠ gefahren“ erweitert, zwei Beispiel-Funde im Mockup; **vierte Rückmeldung: „alle aufnehmen“** – vollständige Liste von 23 Regeln (Anzeige gegen Roboter, Bedienung und Planer, Roboter und Technik) in der Karte F.2, im Mockup als aufklappbare Übersicht; **fünfte Rückmeldung: aus dem „Prompt für Claude“ wird ein Ticketsystem** (Herberts Vorschlag; Claude: besser als ein loser Prompt, weil Status, Wiederkehr-Erkennung und gesicherte Beweise – Bedingungen: ein Ticket je Problem, ClickUp erst beim Bearbeiten, ein einziger Schreibweg, Skill für den immer gleichen Ablauf); Nummern `HT-0001` … (Auswahlfrage), Mockup umgebaut: Kachel „Tickets“, Ticketliste mit Filter, Ticket-Ansicht statt Prompt-Dialog; F.2 geteilt in F.2a Backend / F.2b Karte / F.2c Skill; **zweite Rückmeldung Herbert (18.09.): es fehlte die Auswertung, die Fehler selbst anzeigt und als Prompt für Claude aufbereitet** → Kachel „Auffälligkeiten“ (drei Beispiele aus den echten Befunden des Tages) und Dialog „Prompt für Claude“ mit „Kopieren“, auch an jeder Meldung und als Tagesbericht; Daten über Dienstaufruf mit Antwort statt Datei unter `/local` (der Ordner ist ohne Anmeldung lesbar). Entscheidung Herbert: Mockup zuerst (Auswahlfrage 18.09.), Abnahme offen
- [2026-09-18] [F.1] Entscheidung Herbert · Info (**Erfassung zusätzlich nach Gerät**, Frage Herbert „was ist, wenn neue Entitäten hinzukommen – ist was hardcoded?“): Die Automation nimmt jetzt neben dem Namensmuster `heidi` alle Entitäten des Roboter-Geräts (`device_entities(device_id('vacuum.heidi'))`, heute 239) und Dienstaufrufe an sie – unabhängig vom Namen, sicher gegen Umbenennen einzelner Entitäten. Fest bleiben: `vacuum.heidi` als Anker (wie im ganzen Backend), `input_boolean.stuehle_am_boden`, die drei Rausch-Ausnahmen (Kameras, Zähler der Automationen, `rest_min`) und der Log-Filter `dreame_vacuum`. Eingespielt ohne Neustart (automation reload), Probe ok, keine Fehler im HA-Log. Entscheidung Herbert: ja, einbauen (18.09.)
- [2026-09-18] [Plan] Befund · Info (**Altlasten-Prüfung auf dem Pi**, Frage Herbert „Reste von v1 wie Planer, Szenen – gehören die gelöscht?“): Planer, App-Szenen, alle 5 Skripte, 11 Automationen und die Helfer sind **keine** v1-Reste, sondern das Backend, auf dem v2 läuft (Abschnitt 4); keine verwaisten Heidi-Entitäten. Echte Altlasten: `H:\automations.yaml.v1.bak`, `H:\scripts.yaml.v1.bak`, `H:\packages\heidi.yaml.v1.bak`; vier HACS-Karten samt Ressourcen (Mushroom, card-mod, expander-card, stack-in-card = DX-063; `dreame-vacuum-map-card` und `xiaomi-vacuum-map-card` bleiben, v2 nutzt sie als Kartendarstellung); Kandidat: HA-Theme `heidi_dark/light` (`themes/heidi.yaml`, Skript `heidi_theme_anwenden`, Automation `heidi_dark_mode`) – färbt nur den HA-Rahmen. Nicht Heidi: 33 verwaiste Einträge anderer Integrationen (Pool, Drucker, Mähroboter, Xbox). Entscheidung Herbert: Aufräumen später (DX-063 bleibt offen)
- [2026-09-18] [2.7] Befund · Info: Herbert hat Python 3.14.7 auf dem PC installiert; `test_diag.py` läuft damit auch lokal (12 grün, `cd ha/prognose; python -m unittest discover -s tests -p test_diag.py`). `pytest` fehlt noch (`python -m pip install pytest`, macht Herbert) – danach ist DX-019 / 2.7 ausführbar.
- [2026-09-18] [F.1] Entscheidung Herbert · Info (**Schicht 3 gebaut, PD-017 freigegeben** per Auswahlfrage „Freigeben, jetzt bauen“): Die Karte meldet die sichtbaren Kernwerte des Roboter-Panels (Kopf, Arbeitsschritt, Hinweis-Chip, Akku, Fortschritt, Zustand) als HA-Ereignis `dreame_x60_anzeige`. Schnappschuss und Diff sind reine Funktionen (`src/domain/anzeige.ts`, Eingabe = `RobotView`, also dieselben Werte, die `dx-hero` und der Balken in `dx-auftrag` zeigen); die Shell vergleicht in `updated()` (memoisierte Sicht, kostet nichts bei irrelevanten Ticks), sammelt Änderungen und meldet höchstens einmal je Sekunde (`ANZEIGE_MIN_MS`); jede Meldung trägt Seite, Kartenversion, eine Kennung des Browserfensters (`client`, vier Zeichen je Laden – mehrere Geräte zeigen die Karte gleichzeitig), die Namen der geänderten Werte und alle Werte. Senden nur über `DxApi.fireEvent` (REST `POST events/<typ>`): ohne `hass.user.is_admin`, ohne REST-Zugang oder bei Fehler passiert nichts (kein Wurf, kein Toast). Schalter ist die Kartenoption `diagnose: true` (im Dashboard für alle sechs Ansichten gesetzt) – kein neuer Helfer. Backend: fünfter Auslöser `dreame_x60_anzeige` in `heidi_diagnose_protokoll`, Zeile `art: anzeige` mit `wer` = angemeldeter Benutzer; `tools/diag.js` zeigt sie in der Zeitleiste als „ANZEIGE [Seite · client] wert: …“. Tests: `anzeige.test.ts` (2), `api.test.ts` +1 (Admin/kein Admin/kein REST/Fehler), `hero.js` +8 (42 Prüfungen), `test_diag.py` +1 (12, auf dem Pi grün); 94 Unit, 9 E2E, Lint grün. Version 2.0.0-alpha.32 eingespielt (Ressource per `ha-ws.js`, Automation neu geladen). Ende-zu-Ende geprüft: Ereignis über die API → Zeile mit „Herbert Schrotter“. **Probe Herbert 18.09. 20:39:** Start in der Dreame-App → `tools/diag.js` meldet „Start ← extern … kein HA-Dienstaufruf in den 90 s davor“ (Akzeptanz 1 erfüllt). **Probe Herbert 18.09. 20:48:** Start im Dashboard → Zeile `dienst vacuum.start` mit „Herbert Schrotter“, 0,1 s später `vacuum.heidi docked → cleaning` mit demselben Benutzer-Kontext; `tools/diag.js`: „Start ← Benutzer Herbert Schrotter“ (Akzeptanz 2 erfüllt). Schicht 3 live: Karte (Fenster `4qyf`, alpha.32) meldete „Reinigt / Saugt Staub ab“ → „Reinigt / Fährt zum Startpunkt“ → „Angedockt“ → „Bereit“, jeweils direkt nach dem Wechsel von `vacuum.heidi` bzw. `sensor.heidi_phase`. Beobachtung für später: nach dem kurzen Lauf blieb `fortschritt` 0 statt leer (Sensor `cleaning_progress` war im Stand noch verfügbar) – bei PD-016 am nächsten echten Lauf mit ansehen. Offen: Probe Planerstart (nächster Planerlauf). Entscheidung Herbert: PD-017 freigegeben (18.09.)
...
|---|---|---|---|---|---|---|
| PD-018 | Diagnose / Seite Dev + Fehler melden (F.2) | v1 hat keine Diagnose-Oberfläche | Siebte Seite „Dev“ (nur Admin): Protokoll-Zustand, Starts des Tages mit Quelle, Meldungen, Live-Zeitleiste mit Filter und Suche; Knopf „Fehler melden“ in der Kopfzeile jeder Seite links neben der Uhr (nur Admin) → Ereignis `dreame_x60_meldung` → Zeile `art: meldung`. Automatische Auswertung (23 Regeln in `diag.py`) und **Ticketsystem**: jede Meldung und jeder Fund wird ein Ticket `HT-NNNN` mit gesicherten Beweisen und Status (neu → angenommen → in Arbeit → gelöst → geschlossen, verworfen); Kachel und Liste „Tickets“, Ticket-Ansicht; Bearbeitung einzeln in Claude Code mit dem Skill `ticket`, ClickUp-Aufgabe erst beim Bearbeiten. Neu in Abschnitt 4 zusätzlich: Dienst `shell_command.heidi_ticket` (L/S, mit Antwort). Dev steht in Seitenleiste, Symbolleiste und als sechster Platz in der Tab-Leiste am Handy (nur Admin). Neu in Abschnitt 4: Ereignis `dreame_x60_meldung` (S), Dienst `shell_command.heidi_diag_tail` (L, mit Antwort) | Herberts Wunsch (18.09.): das Protokoll braucht eine Oberfläche und einen Knopf zum Melden; neue Funktion (Ausnahme von Regel 2) | `dev.js`, `api.test.ts`, `test_diag.py` | freigegeben (Herbert, 2026-09-18, Mockup `dreame_x60/mockups/dev.html` abgenommen) |
| PD-017 | Diagnose / Kartenanzeige (F.1 Schicht 3) | v1 meldet nichts an HA zurück | Die Karte feuert das HA-Ereignis `dreame_x60_anzeige`, wenn sich die sichtbaren Kernwerte des Roboter-Panels ändern (Kopftext, Arbeitsschritt, Fortschritt, Akku, Hinweis-Chip; höchstens einmal je Sekunde, nur geänderte Werte, dazu Seite und Kartenversion); Automation `heidi_diagnose_protokoll` schreibt es als `art: anzeige` mit. Nur über `DxApi` (neue Methode `fireEvent`), nur wenn die Kartenoption `diagnose: true` im Dashboard gesetzt ist, und nur bei Admin-Benutzern (HA erlaubt Ereignisse aus dem Frontend sonst nicht) – bei anderen Benutzern still ohne Fehler. Neu in Abschnitt 4: Ereignis `dreame_x60_anzeige` (S). Kein neuer Helfer | Herberts Aufgabe DX-068 (18.09.): Vergleich „was zeigte das Dashboard wann“ gegen den tatsächlichen Stand; neue Funktion (Ausnahme von Regel 2) | `anzeige.test.ts`, `api.test.ts` (fireEvent, Nicht-Admin = kein Aufruf), `hero.js` (Meldung je Wertwechsel, keine bei irrelevanten Ticks, keine ohne `diagnose: true`, keine ohne Admin-Recht) | freigegeben (Herbert, 2026-09-18), umgesetzt 18.09. (alpha.32) |
