...
- [2026-09-18] [F.2] Wunsch · Functional (Herbert nach der Probe: „gibt es keine UI und ein neues Tab Dev? und den Button um Fehler zu melden?“): War nicht Teil von F.1/DX-068 (dort nur Protokoll, Kartenmeldung, Auswertung per Skript). Neue Aufgabe **F.2 / DX-069**, Karte in Abschnitt 8, PD-018 offen, Mockup `dreame_x60/mockups/dev.html` (Desktop 1180 px + 390 px; Zustände: Seite Dev, Dialog offen, nach dem Senden, Benutzer ohne Admin-Recht; Token-Abgleich mit `tokens.ts` ohne Abweichung). Entscheidungen im Mockup: Dev nur für Admin; am Handy keine sechste Tab-Leisten-Position, Zugang über Einstellungen; Daten über Dienstaufruf mit Antwort statt Datei unter `/local` (der Ordner ist ohne Anmeldung lesbar). Entscheidung Herbert: Mockup zuerst (Auswahlfrage 18.09.), Abnahme offen
...
|---|---|---|---|---|---|---|
| PD-018 | Diagnose / Seite Dev + Fehler melden (F.2) | v1 hat keine Diagnose-Oberfläche | Siebte Seite „Dev“ (nur Admin): Protokoll-Zustand, Starts des Tages mit Quelle, Meldungen, Live-Zeitleiste mit Filter und Suche; Knopf „Fehler melden“ in der Kopfzeile jeder Seite (nur Admin) → Ereignis `dreame_x60_meldung` → Zeile `art: meldung`. Handy: Tab-Leiste unverändert, Zugang über Einstellungen. Neu in Abschnitt 4: Ereignis `dreame_x60_meldung` (S), Dienst `shell_command.heidi_diag_tail` (L, mit Antwort) | Herberts Wunsch (18.09.): das Protokoll braucht eine Oberfläche und einen Knopf zum Melden; neue Funktion (Ausnahme von Regel 2) | `dev.js`, `api.test.ts`, `test_diag.py` | **offen** – Mockup `dreame_x60/mockups/dev.html` 18.09., Abnahme Herbert steht aus |
