...
- Akzeptanz: Seite Dev zeigt die letzten Zeilen, neue erscheinen ohne Neuladen (Live an), Filter und Suche wirken; „Fehler melden“ schreibt genau eine Zeile `art: meldung` mit Text, Benutzer, Seite, Version, Anzeige-Schnappschuss; Nicht-Admin: kein Eintrag Dev, kein Knopf, kein Fehler; leere Meldung wird nicht gesendet; Fehler beim Senden → Toast, Dialog bleibt offen (Regel 20).
- Tests: `dev.js` (E2E: Zeitleiste aus Stub-Antwort, Filter, Live-Abruf, Melden → ein Ereignis, Nicht-Admin), `api.test.ts` (diagTail, reportProblem), `test_diag.py` (tail, Art meldung), `npm test`, `npm run lint`
