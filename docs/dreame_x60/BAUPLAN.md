...
|---|---|---|---|---|---|---|
| PD-017 | Diagnose / Kartenanzeige (F.1 Schicht 3) | v1 meldet nichts an HA zurück | Vorschlag: Die Karte feuert das HA-Ereignis `dreame_x60_anzeige`, wenn sich die sichtbaren Kernwerte des Roboter-Panels ändern (Kopftext, Arbeitsschritt, Fortschritt, Akku, Hinweis-Chip; höchstens einmal je Sekunde, nur geänderte Werte, dazu Seite und Kartenversion); Automation `heidi_diagnose_protokoll` schreibt es als `art: anzeige` mit. Nur über `DxApi` (neue Methode `fireEvent`), nur wenn die Kartenoption `diagnose: true` im Dashboard gesetzt ist, und nur bei Admin-Benutzern (HA erlaubt Ereignisse aus dem Frontend sonst nicht) – bei anderen Benutzern still ohne Fehler. Neu in Abschnitt 4: Ereignis `dreame_x60_anzeige` (S). Kein neuer Helfer | Herberts Aufgabe DX-068 (18.09.): Vergleich „was zeigte das Dashboard wann“ gegen den tatsächlichen Stand; neue Funktion (Ausnahme von Regel 2) | `api.test.ts` (fireEvent, Nicht-Admin = kein Aufruf), `hero.js` (ein Ereignis je Wertwechsel, keines bei irrelevanten Ticks, keines ohne `diagnose: true`) | **offen** – Entwurf 18.09., Freigabe Herbert steht aus |
