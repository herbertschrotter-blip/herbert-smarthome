...
- **4.3e Einrichtungsprüfung (16.09., PD-014):** in der Kopfzeile zwischen Titel und Uhr nur die Symbole mit Befund (rot pulsiert, Klick springt direkt zur Stelle; `dx-setup`-Liste für 4.11 vorbereitet) – acht Prüfungen inkl. Räume ↔ HA-Bereiche (Entitäts-Register `options.vacuum.area_mapping` über callWS) und HA-Reparaturen; Klick springt zur Stelle. Version 2.0.0-alpha.23 (Symbole rechtsbündig neben der Uhr). Herbert hat alle sieben Räume HA-Bereichen zugeordnet (Bad, WC, Büro neu; Corridor → Vorraum).
