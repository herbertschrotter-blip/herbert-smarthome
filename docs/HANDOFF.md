...
- [ ] **DX-068 (F.1) Diagnose-Protokoll – Schicht 1 + 2 laufen seit 18.09. 20:27** (ClickUp in development, Titel/Template/Felder nachgezogen). Schicht 3 (PD-017) seit 18.09. als alpha.32 eingespielt. Offen: (a) **Probe:** App-Start = „extern“ ist bestätigt (18.09. 20:39); fehlen Dashboard-Start („Benutzer Herbert Schrotter“) und Planerstart („Automation Heidi: Planer“) → `node tools\diag.js`; (b) nach ein paar Tagen Größe von `H:\prognose\diag\` ansehen; (c) danach `tracker done` DX-068.
