...
- **4.3e Einrichtungsprüfung (16.09., PD-014):** `dx-setup` über der Übersicht nur bei Befund – acht Prüfungen inkl. Räume ↔ HA-Bereiche (Entitäts-Register `options.vacuum.area_mapping` über callWS) und HA-Reparaturen; Klick springt zur Stelle. Version 2.0.0-alpha.20. Herbert hat alle sieben Räume HA-Bereichen zugeordnet (Bad, WC, Büro neu; Corridor → Vorraum).
