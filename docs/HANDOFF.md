...
## 3e. Projekt dreame_x60 – Neubau der Karte (seit 14.09.2026; letzter Stand 19.09.2026, Sitzung Teil 7: Version 2.0.0-alpha.41, PD-020 Ortung, HT-0010)
...
- **F.2b Karte (18.09., Version 2.0.0-alpha.33 eingespielt):** Seite **Dev** (nur Admin; Seitenleiste, Symbolleiste, Tab-Leiste) mit Protokoll-Zustand, Starts des Tages, Tickets, Regelübersicht und Live-Zeitleiste (`dx-dev`), Ticket-Ansicht (`dx-ticket`), Knopf **„Fehler melden“** links neben der Uhr auf jeder Seite (`dx-report` → Ereignis `dreame_x60_meldung` → Ticket), erweiterte Anzeige-Meldung (Knöpfe, Laden, Werte-Knöpfe als HA-Werte, „Jetzt“, Reihenfolge) mit Lebenszeichen alle 30 s und `ende`. Tests 100 Unit, 10 E2E (neu `dev.js` 29), Lint grün; Meldeweg und Dienste mit Antwort am echten HA geprüft. Nachbesserung alpha.34 (Herberts Sichtprüfung): „Kopieren“ in der Ticket-Ansicht funktioniert jetzt auch über http – Stolperstein: `navigator.clipboard` gibt es nur unter https/localhost, HA im Heimnetz ist kein gesicherter Kontext (`src/shared/clipboard.ts`). Nachbesserung alpha.35: „Starts heute“ kommt jetzt vom Backend aus dem ganzen Tag und kennt die Startfolge `cleaning → docked → idle → cleaning` (Halt unter 45 s = derselbe Lauf, `GAP_S`) – vorher erschien ein Dashboard-Start als „extern“; Ticket-Filter um „In Arbeit“ und „Verworfen“ ergänzt. **Offen:** weitere Sichtprüfung Herbert (Strg+F5: Dev-Seite, „Fehler melden“, Handy-Tab-Leiste).
- **F.2c Skill `ticket` (18.09.):** im Skills-Repo (`D:\OneDrive\Dokumente\02 Arbeit\05 Vorlagen - Scripte\00_claude-skills-bpm`, v0.36.0) – Tickets einzeln in acht festen Schritten, jeder Übergang Auswahlfrage; Projektwerte im **Ticket-Profil** der CLAUDE.md; Werkzeug `.\tools\ticket.ps1 liste|zeige|status|notiz|verwerfen|auswertung`. Ticket-Stand 18.09.: HT-0001 offen (Integrationsfehler „Map render Failed“, 104×), HT-0004 gelöst (Fehler in Regel A5, Commit 94be28f – Schließen durch Herbert offen), HT-0002/HT-0003 verworfen (Proben). **Offen:** Skill bei claude.ai hochladen (Herbert), Live-Trigger-Tests, erste Probe „ticket HT-0001“.
- **Einspielen der Karte (19.09., Version 2.0.0-alpha.38):** `tools/deploy.ps1` schreibt jede Datei erst vollständig unter einem Hilfsnamen und benennt dann in einem Zug um; braucht PowerShell 7. **Stolperstein:** Ruft der Browser die Karten-Datei ab, während sie kopiert wird, behält er die halbe Datei wochenlang im Cache → Dashboard zeigt nur „Konfigurationsfehler“ (so geschehen bei alpha.37; Abhilfe: Ressourcen-Adresse ändern). Reihenfolge: einspielen, prüfen, **dann** Ressourcen-Version setzen; `deploy.ps1` nie über `powershell.exe` (5.1) aufrufen. alpha.38 ist inhaltlich gleich alpha.37 (nur neue Adresse).
- **HT-0001 / DX-070 (18.09., erstes Ticket nach dem festen Ablauf):** Das Kartenbild blieb im Lauf stehen, weil die Dreame-Integration (v2.0.0b25) beim Vergleich der Räume abbricht, sobald es einen Raum ohne Koordinaten gibt (versteckter Raum 8 / Balkon). Örtliche Korrektur: eine Zeile `self.area = None` in `H:\custom_components\dreame_vacuum\dreame\types.py` (Sicherung `types.py.ht0001.bak`), HA-Neustart 22:33. **Stolperstein:** Ein HACS-Update der Integration überschreibt die Korrektur → danach `.\tools\check-dreame-patch.ps1` ausführen (Exit 1 = fehlt); das Ticket öffnet sich sonst von selbst neu. Fehlerbericht für den Entwickler: `docs/dreame_x60/upstream/HT-0001-segment-area.md` (nicht abgeschickt – Stand 18.09.2026: bei Tasshack lassen sich keine Issues anlegen (vom Repo-Betreiber gesperrt), Herbert lässt es; die örtliche Korrektur bleibt die Lösung). Erklärt sehr wahrscheinlich auch „Reinigungspfad fehlt im Kartenbild“ (15.09.). Endgültige Probe mit verstecktem Raum 8 bestanden (17 Bilder, 0 Abbrüche) → HT-0001 `geloest`, DX-070 testing. **Offene Tickets für die nächsten Durchläufe (`.\tools\ticket.ps1 liste`):** keine mehr (Stand 19.09.); **HT-0006 gelöst (19.09., alpha.39, PD-019):** Im Lauf zeigte die Karte immer die Raum-Werte, auch wenn „Angepasste Reinigung“ aus war (App-Start) und der Roboter mit seinen allgemeinen Werten fuhr – jetzt „wirksame Werte“ (`wirksameWerte()`, Attribut `customized_cleaning` von `vacuum.heidi`) in Kopf, Streifen, Auftrag-Kachel und Bericht, Hinweis „Allgemeine Werte“; Sichtprüfung Herbert beim nächsten App-Start offen; **HT-0008 gelöst und geschlossen (19.09.):** Fehlalarm „Lade-Anzeige“ nach HA-Neustart – während HA startet, schreibt das Protokoll nicht mit, die Auswertung arbeitete mit veralteten Werten; jetzt Zeile `neustart` mit Schnappschuss beim HA-Start (`heidi_diagnose_protokoll`) und `Welt.neustart()` in `diag_regeln.py`; Probe mit echtem HA-Neustart 19.09. 09:39 bestanden (Zeile im Tab Dev: „Home Assistant neu gestartet“, 0 Funde); **Bug-Schema seit 19.09.:** Tickets stehen in ClickUp in der eigenen Liste „dreame_x60 – Tickets“ als `HT-NNNN | KÜRZEL | Kurztitel` mit Tag `bug`, ohne DX-Nummer (Ticket-Profil der CLAUDE.md; DX-070 → HT-0001, DX-072 → HT-0007 umgestellt); **HT-0005 gelöst (19.09., alpha.37):** die Karte meldet den Fortschritt nur noch, wenn der Balken zu sehen ist (Fehlalarm beim Start: Sensor 0, Roboter noch in der Station); **HT-0007 gelöst und geschlossen (18.09.):** „Saugt Staub ab“ in der Station – der Absaug-Sensor meldet nach kurzen Läufen `not_performed`, die Phasen-Vorlage kennt jetzt nur noch `active` als Absaugen (auch Regel A8); neuer Test `test_diag_phase.py` rechnet die echte Vorlage durch (nur auf dem Pi, braucht jinja2) – je ein Ticket zur Zeit mit dem Skill `ticket`.
- **PD-020 Ortung gebaut (19.09., Sitzung Teil 7, 4.1-Nachtrag, DX-026; Version 2.0.0-alpha.40 eingespielt):** Entitäten-Durchgang fortgesetzt – 3. `sensor.heidi_mapping_time` **nicht integrieren** (Herbert), 4. `sensor.heidi_relocation_status` **gebaut**: im Lauf steht bei `locating` „Sucht Position …“ als Arbeitsschritt, bei `failed` (ohne Fehlercode) gelber Hinweis „Position unbekannt“ (Antippen → more-info des Ortungs-Sensors). Neu im Vertrag: `relocationStatus`. **Stolperstein:** Regel A1 der Auswertung vergleicht den Arbeitsschritt der Karte mit `sensor.heidi_phase` – jeder neue Arbeitsschritt-Text der Karte braucht dort eine Ausnahme (hier `ORTUNG_SUCHT` in `diag_regeln.py`), sonst entsteht ein Fehlalarm-Ticket. 5. `sensor.heidi_state` → Post-2.0 (Bauplan Abschnitt 10). 6. `sensor.heidi_stream_status` → Post-2.0 („Kamera aktiv“). 7. `sensor.heidi_task_type` → Post-2.0 für die Diagnose (Start-Quelle „extern“ verfeinern). 8. `switch.heidi_resume_cleaning` → vorgemerkt für die Roboter-Einstellungen (Modul E, Karte 4.7). Gruppe „Zustand und Lauf“ ist durch; weiter mit der Gruppe „Akku und Laden“ (9. `binary_sensor.heidi_charging_state`).
- **HT-0010 gelöst (19.09., Version 2.0.0-alpha.41):** Die Karte der Übersicht war unten abgeschnitten, seit das Kartenbild quadratisch ist – HA gibt `picture-entity` im Live-Modus ohne `aspect_ratio` einen festen 16:9-Rahmen. Die Karte misst jetzt die Bildgröße (`ha/map-ratio.ts`) und gibt das Format mit; Rahmen höchstens 420 px hoch (`MAP_COMPACT_MAX_HEIGHT_PX`), hohes Bild mittig. **Stolperstein:** `fit_mode: contain` wirkt im Live-Modus nicht; der eingebaute Browser von Claude Code ist in HA angemeldet – damit lässt sich die echte Oberfläche messen (nur lesen). HT-0009 (Cloud-Aussetzer 2× je 10 s) verworfen.
- **HT-0012 gelöst (19.09., Backend):** Fehlalarm „Reihenfolge passt nicht“ – Regel A3 zählte Durchfahrten (7 s Flur auf dem Weg ins Wohnzimmer) als gefahren; jetzt zählt ein Raum erst ab 45 s Aufenthalt (`DURCHFAHRT_S`), Zeit in der Station nicht, Fortsetzung mitten im Raum ab dem Start. HT-0011 (festgefahren, `forward_suffocate`) verworfen.
...
## 3f. Netzdiagnose (19.09.2026, unabhängig von Heidi)
Anlass: Seit Tagen verlieren einzelne Geräte (LAN und WLAN durcheinander) das Internet, Verdacht Adress-Konflikte.
- **Skript** `ha/netzdiagnose/fritz_diag.py` (nur Standardbibliothek, 11 Tests unter `tests/`): fragt jede Minute die FRITZ!Box 6860 5G
  über TR-064 ab (Zugangsdaten aus dem vorhandenen Eintrag der Integration „FRITZ!Box Tools“, kein Kennwort im Repo oder in YAML).
  Sichert das Ereignisprotokoll der Box dauerhaft (`data/fritzlog.jsonl` – die Box vergisst es beim Neustart), vergleicht die
  Gerätetabelle (`data/journal.jsonl`: neu, IP-Wechsel, online/offline, LAN↔WLAN) und meldet Befunde: doppelte IP, feste IP im
  DHCP-Bereich (192.168.170.20–200), Adresse außerhalb des Heimnetzes, Internetsperre durch Zugangsprofil, zweiter DHCP-Server
  (eigene DHCP-Anfrage alle 10 min über Rohdaten-Socket, erfundene Adresse `02:4E:45:54:5A:44`, es wird nie eine Adresse angenommen),
  Wechsel der Hardware-Adresse der Box, Pi → Box/Internet/DNS. `data/` liegt nur auf dem Pi, nie im Repo.
- **HA:** `ha/packages/netzwerk.yaml` (4 command_line-Sensoren `sensor.netzdiag_status|journal|fritzlog|geraete`, Listen vom Recorder
  ausgenommen, Automation `netzdiag_meldung_kritisch` → Benachrichtigung), Dashboard `ha/dashboards/netzwerk.yaml`
  (Seitenleiste „Netzwerk“, `/netz-diagnose`, Markdown-Tabellen – `{%- … %}` mit Bindestrich, sonst zerfällt die Tabelle).
- **Stand 19.09. 10:14:** kein Befund, nur die Box antwortet als DHCP-Server, alle 12 Online-Geräte beziehen per DHCP; 21 bekannte
  Geräte offline (Adressquelle wird erst geprüft, wenn sie online sind). **Offen:** beim nächsten Ausfall Journal + Befunde auswerten.

## 4. Offene Punkte (Stand 19.09.2026, Ende Claude-Code-Sitzung Teil 6; ClickUp-Bezug je Punkt, Audit 16.09.)
Erledigt:
...
- [ ] **Entitäten-Durchgang (Modul R, ohne eigene ClickUp-Aufgabe, läuft unter DX-026/DX-064):** erledigt bis 4. (`mapping_time` nicht integrieren, `relocation_status` gebaut als PD-020, alpha.40); 5. `sensor.heidi_state` Post-2.0; 6. `sensor.heidi_stream_status` Post-2.0; 7. `sensor.heidi_task_type` Post-2.0 (Diagnose); 8. `switch.heidi_resume_cleaning` vorgemerkt für Modul E; Gruppe „Zustand und Lauf“ durch, stehen geblieben vor „Akku und Laden“ (9. `binary_sensor.heidi_charging_state`); Stand in ENTITAETEN.md (Kopf) und Bauplan Abschnitt 10 (19.09.).
- [ ] **Sichtprüfung Herbert nach Strg+F5 (Version 2.0.0-alpha.41; HT-0010: Karte der Übersicht ganz sichtbar, nicht mehr abgeschnitten; PD-020 „Sucht Position …“ zeigt sich nur, wenn Heidi im Lauf ihre Position sucht – z. B. nach Hochheben oder „Fortsetzen“; neu sichtbar: Seite Dev, Knopf „Fehler melden“; **DX-071:** Live-Karte der Übersicht läuft als HA-Live-Ansicht – von Herbert am 18.09. im Lauf geprüft („läuft flüssig“)):** Roboter-Panel (DX-026), Texte unverändert nach 4.14 (DX-066); Warnungs-Chip mit ✕ und Fortschrittsbalken erst beim nächsten echten Fall bzw. Lauf sichtbar – dabei prüfen, wie der Roboter den Fortschritt bei Raumaufträgen rechnet (PD-016).
- [ ] **DX-069 (F.2) Seite „Dev“, Tickets, „Fehler melden“:** alle drei Teilschritte gebaut 18.09. (F.2a Backend, F.2b Karte alpha.33, F.2c Skill `ticket`), ClickUp in testing. Offen für Herbert: Sichtprüfung nach Strg+F5, Skill `ticket` bei claude.ai hochladen.
- [ ] **Tickets (Stand 19.09., `.\tools\ticket.ps1 liste alle`): kein Ticket offen.** Geschlossen: HT-0001, HT-0007, HT-0008. **Gelöst, Prüfung Herbert offen:** HT-0004 (Regel A5, seit 18.09. nicht mehr aufgetreten → schließen), HT-0005 und HT-0006 (beide zeigen sich bei **einem Start aus der Dreame-App**: kein neuer Fund „Fortschritt 0 %“, Knöpfe zeigen bei ausgeschalteter „Angepasster Reinigung“ die allgemeinen Werte + Hinweis „Allgemeine Werte“ → dann schließen). Verworfen: HT-0002, HT-0003. ClickUp-Aufgaben der Tickets stehen in der Liste „dreame_x60 – Tickets“ (alle testing; Abnahme auf shipped macht Herbert).
- [ ] **ClickUp-Liste „dreame_x60 – Tickets“ (neu 19.09.) [Herbert]:** (a) die Zusatzfelder der Bauplan-Liste (Typ, Aufwand, Zielversion, Komponente, Zugehörige Docs, Commit ID, Commit Text, Erledigt, Chat-Anker …) in ClickUp über „Vorhandenes Feld hinzufügen“ an die Liste hängen – bis dahin stehen Commit und Version als Kommentar an den Aufgaben, Claude füllt danach nach; (b) optional den Aufgabentyp „Bug“ im Workspace anlegen (bisher Tag `bug`).
- [ ] **DX-071 (4.3 Live-Karte der Übersicht):** gebaut und von Herbert am 18.09. im Lauf geprüft („läuft flüssig“), ClickUp testing → Abnahme auf shipped durch Herbert.
- [ ] **Nach jedem HACS-Update der Dreame-Integration:** `.\tools\check-dreame-patch.ps1` (HT-0001; Stand 19.09. nach HA-Neustart: Korrektur vorhanden, v2.0.0b25).
