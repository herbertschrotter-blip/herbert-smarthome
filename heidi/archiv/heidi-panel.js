...
/* Dauer & Akku */
.estline { border: 0; background: transparent; color: var(--muted); padding: 0; margin-top: 2px; font: inherit; font-size: 11px; font-weight: 600; display: inline-flex; align-items: center; gap: 2px; cursor: pointer; white-space: nowrap; }
...
class HeidiPanel extends HTMLElement {
  ...
  _estHtml() {
    ...
    const sum = `<div class="esum ${fits ? "ok" : "bad"}"><div><b>${this._fmtMin(e.total)}</b> · fertig um ${tm(new Date(now.getTime() + e.total * 60000))} bei Start jetzt</div>
      <div>${rest.min > 0 ? `Rückkehr in ${this._fmtMin(rest.min)} (${esc(rest.quelle)}) → <b>${fits ? "reicht" : "reicht nicht"}</b>` : "Rückkehrzeit vorbei"}${e.charges ? ` · ${e.charges}× Nachladen` : ""} · Akku ${Math.round(e.batt0)} % → ${Math.round(e.battEnd)} %${e.unlearned ? " · teils geschätzt" : ""}</div>
      ${p.schnell ? (() => { const s = this._estimate(p, "schnell"); return `<div>Schnellprogramm: <b>${this._fmtMin(s.total)}</b> · ${s.total <= rest.min ? "reicht" : "reicht nicht"}</div>`; })() : ""}</div>`;
