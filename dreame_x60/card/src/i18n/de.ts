// Deutsche Texte der Karte (Bauplan 4.14, i18n Stufe 1). Einziger Ort für feste Texte: Bausteine, Shell, Domäne und
// Tabellen lesen über t()/tx()/lookup() aus ./t.ts. Wortlaut wie bisher (v1-Parität, Vektoren unverändert).
// Schlüssel: <bereich>.<name>, Platzhalter als {name}. Werte aus HA (Raumnamen, Planer-Namen, Personen, Zahlen, Datum)
// stehen nicht hier – nur ihre Übersetzungen (status.*, task.*, error.*, option.*, room.*, roomtype.*).
// Gruppen: common, nav, page, slot, topbar, hero, auftrag, quickstart, map, mapmode, dialog, setup, rooms, reinigen,
// preview, status, task, button, label, day, estimate, strip, option, room, roomtype, settings, prognose, station,
// consumable, robotsettings, diag, automatik, card, error, errorLong.
export const de = {
  // ── Allgemein ──
  'common.robot': 'Roboter',
  'common.dash': '–',
  'common.ok': 'OK',
  'common.cancel': 'Abbrechen',
  'common.back': 'Zurück',
  'common.close': 'Schließen',
  'common.open': 'Öffnen',
  'common.start': 'Starten',
  'common.on': 'an',
  'common.off': 'aus',
  'common.active': 'aktiv',
  'common.inactive': 'inaktiv',
  'common.none': 'keine',
  'common.present': 'vorhanden',
  'common.missing': 'fehlt',
  'unit.min': 'min',
  'unit.m2': 'm²',

  // ── Navigation (dx-nav, config NAV) ──
  'nav.start': 'Übersicht',
  'nav.reinigen': 'Karte',
  'nav.rooms': 'Räume',
  'nav.planer': 'Planer',
  'nav.protokoll': 'Verlauf',
  'nav.prognose': 'Prognose',
  'nav.einstellungen': 'Einstellungen',
  'nav.dev': 'Dev',
  'nav.brandSub': 'Dein Saugroboter',
  'nav.model': 'Dreame X60 Ultra',
  'nav.sidebar': 'Seitenleiste',
  'nav.tabbar': 'Tab-Leiste',

  // ── Seiten (pages.ts) ──
  'page.start.sub': 'Übersicht',
  'page.reinigen.title': 'Karte',
  'page.reinigen.sub': 'Räume, Zone oder Punkt reinigen · Hinfahren · Sperrzonen',
  'page.planer.title': 'Planer',
  'page.planer.sub': 'Vier Einträge · Automatik entscheidet voll, schnell oder warten',
  'page.protokoll.title': 'Verlauf',
  'page.protokoll.sub': 'Reinigungsprotokoll der App · Zeitleiste je Lauf · Lernwerte',
  'page.prognose.title': 'Prognose',
  'page.prognose.sub': 'Lernende Anwesenheit · Grundlage für Start, Schnellprogramm und Rückkehr',
  'page.einstellungen.title': 'Einstellungen',
  'page.einstellungen.sub': 'Darstellung, Funktionen, Prognose, Roboter, Diagnose',
  // Platzhalter je Unterseite: welche Bausteine hier laut Bauplan Abschnitt 7 entstehen (verschwinden mit den Modulen)
  'page.dev.title': 'Dev',
  'page.dev.sub': 'Diagnose-Protokoll – was war wann, und wer hat es ausgelöst',
  // ── Seite Dev, Tickets, Fehler melden (F.2b, PD-018) ──
  'dev.log.title': 'Protokoll',
  'dev.log.running': 'läuft · letzte Zeile vor {sek} s',
  'dev.log.empty': 'noch keine Zeilen',
  'dev.log.error': 'Protokoll nicht lesbar: {text}',
  'dev.log.lines': 'Zeilen heute · {kb} KB',
  'dev.log.l1': 'Zustände',
  'dev.log.l2': 'Debuglog',
  'dev.log.l3': 'Kartenanzeige',
  'dev.log.keep': 'Aufbewahrung 30 Tage (Debuglog 14) · Auswertung alle 10 Minuten',
  'dev.starts.title': 'Starts heute',
  'dev.starts.row': '{zeit} · Start',
  'dev.starts.noCall': 'kein HA-Aufruf in den 90 s davor',
  'dev.starts.none': 'Heute noch kein Start.',
  'dev.starts.hint': 'extern = Dreame-App, Knopf am Roboter oder App-Zeitplan',
  'dev.quelle.benutzer': 'Benutzer',
  'dev.quelle.automation': 'Automation',
  'dev.quelle.system': 'System',
  'dev.quelle.extern': 'extern',
  'dev.src.card': 'Karte · {seite}',
  'dev.src.report': 'Meldung · {wer}',
  'dev.src.user': 'Benutzer {wer}',
  'dev.src.auto': 'Automation {name}',
  'dev.src.autoPlain': 'Automation',
  'dev.src.system': 'System (Zeit)',
  'dev.src.extern': 'extern',
  'dev.row.call': 'Dienst {dienst}',
  'dev.row.shows': 'zeigt',
  'dev.row.page': 'Seite {seite}',
  'dev.tickets.title': 'Tickets',
  'dev.tickets.new': 'neu',
  'dev.tickets.working': 'in Arbeit',
  'dev.tickets.solved': 'gelöst',
  'dev.tickets.flow': 'neu → angenommen (DX-…) → in Arbeit → gelöst → geschlossen · verworfen',
  'dev.tickets.hint': 'Bearbeiten in Claude Code: ticket HT-NNNN – einzeln, mit dir zusammen, nichts automatisch.',
  'dev.tickets.none': 'Keine Tickets in dieser Ansicht.',
  'dev.tickets.fromReport': 'Meldung',
  'dev.tickets.fromRule': 'Auswertung · Regel {regel}',
  'dev.tickets.count': '{n}× · zuletzt {zeit}',
  'dev.tickets.again': '{n}× wieder aufgetreten',
  'dev.tfilter.offen': 'Offen',
  'dev.tfilter.geloest': 'Gelöst',
  'dev.tfilter.alle': 'Alle',
  'dev.status.neu': 'neu',
  'dev.status.angenommen': 'angenommen',
  'dev.status.in_arbeit': 'in Arbeit',
  'dev.status.geloest': 'gelöst',
  'dev.status.geschlossen': 'geschlossen',
  'dev.status.verworfen': 'verworfen',
  'dev.rules.title': 'Alle Regeln der Auswertung (23) – was automatisch bemerkt wird',
  'dev.rules.a.title': 'Anzeige gegen Roboter',
  'dev.rules.a.items': 'Kopf / Arbeitsschritt ≠ Roboter länger als 10 s|Werte-Knöpfe Modus, Saugstufe, Wasser ≠ gültige Werte des Laufs|angekündigte Reihenfolge ≠ gefahrene Räume|Raum-Chip / „Jetzt“ ≠ aktueller Raum|Knöpfe passen nicht zum Zustand|Akku oder Lade-Blitz ≠ Roboter|Fortschritt außerhalb eines Laufs|Station: Karte „Bereit“, während gewaschen, getrocknet oder abgesaugt wird (ab Modul C)|Warnung des Roboters ohne Chip|Karte bekommt keine Daten mehr',
  'dev.rules.b.title': 'Bedienung und Planer',
  'dev.rules.b.items': 'Knopf / Dienstaufruf ohne Wirkung in 30 s|gewählte Räume ≠ Auftrag des Roboters|Planer sagt „startet“, aber kein Start|Lauf fertig, „heute erledigt“ nicht gesetzt|Start zur falschen Uhrzeit oder obwohl eine „stört“-Person zu Hause ist|Raum-Werte nach dem Lauf nicht wiederhergestellt|„Angepasste Reinigung“ bleibt nach dem Lauf aus|geschätzte Dauer weicht mehr als 30 % ab (ab 4.8)',
  'dev.rules.t.title': 'Roboter und Technik',
  'dev.rules.t.items': 'Start ohne HA-Auslöser (App, Knopf, App-Zeitplan) – nur Info|Warnung oder Fehler des Roboters|Roboter länger als 10 min nicht erreichbar|Fehler / Traceback der Dreame-Integration|Lücke im Protokoll',
  'dev.rules.app': 'Nicht automatisch prüfbar: was die Dreame-App selbst anzeigt – dafür ist der Knopf „Fehler melden“.',
  'dev.tl.title': 'Zeitleiste',
  'dev.tl.sub': '{n} Zeilen · neueste oben',
  'dev.tl.search': 'Entität oder Text, z. B. vacuum.heidi',
  'dev.tl.live': 'Live',
  'dev.tl.older': 'Ältere laden',
  'dev.tl.hint': 'Je Abruf {n} Zeilen über einen Dienst mit Antwort – keine Datei im öffentlichen Ordner.',
  'dev.filter.all': 'Alles',
  'dev.filter.robot': 'Roboter',
  'dev.filter.call': 'Dienste',
  'dev.filter.auto': 'Automationen',
  'dev.filter.card': 'Anzeige',
  'dev.filter.report': 'Meldungen',
  'api.noWs': 'Keine WebSocket-Verbindung zu Home Assistant',
  'api.noAnswer': 'Keine Antwort vom Dienst',
  'api.ticketError': 'Der Ticket-Dienst meldet einen Fehler',
  'report.title': 'Fehler melden',
  'report.intro': 'Setzt eine Marke ins Protokoll – genau jetzt – und legt ein Ticket an. So finden wir die Stelle später sofort.',
  'report.quick': 'Anzeige stimmt nicht|Roboter reagiert nicht|Planer|Karte / Räume',
  'report.placeholder': 'Was stimmt nicht? (kurz, z. B. „Kopf zeigt Bereit, Heidi fährt aber“)',
  'report.sends': 'Wird mitgeschickt: Seite, Kartenversion, was die Karte gerade zeigt, Zustand des Roboters, dein Benutzer.',
  'report.send': 'Senden',
  'report.empty': 'Bitte kurz beschreiben oder ein Stichwort wählen.',
  'report.failed': 'Senden hat nicht geklappt (nur Admin-Benutzer dürfen melden). Bitte noch einmal versuchen.',
  'report.sent': 'Gemeldet – das Ticket wird angelegt',
  'report.sentNr': 'Ticket {nr} angelegt – Marke steht im Protokoll',
  'ticket.title': 'Ticket',
  'ticket.loading': 'Ticket wird geladen …',
  'ticket.intro': 'Gesicherte Beweise (bleiben erhalten, auch wenn das Protokoll gelöscht wird). In Claude Code: ticket {nr}',
  'ticket.clickup': 'ClickUp {dx}',
  'ticket.noClickup': 'ClickUp: noch keine Aufgabe',
  'ticket.input': 'Notiz – oder der Grund, warum es kein Fehler ist',
  'ticket.needText': 'Bitte zuerst einen Text eingeben.',
  'ticket.reject': 'Verwerfen – kein Fehler',
  'ticket.rejected': 'Ticket {nr} verworfen',
  'ticket.note': 'Notiz speichern',
  'ticket.noted': 'Notiz gespeichert',
  'ticket.copy': 'Kopieren',
  'ticket.copied': 'Ticket {nr} kopiert – in Claude einfügen',
  'ticket.copyFailed': 'Kopieren nicht möglich – Text bitte markieren.',
  'page.parts.reinigen': 'dx-map-card full (4.3)|App-Szenen|Stühle am Boden|Räume (Roboter-Werte) → dx-rooms-dialog (4.6)',
  'page.parts.planer': 'dx-planer (4.4)|dx-planer-editor + dx-clock-picker (4.5)|Automatik-Regeln|dx-estimate-dialog (4.8)',
  'page.parts.protokoll': 'dx-history (4.7)|Lernwerte-Tabelle',
  'page.parts.prognose': 'dx-prognose-view (4.10)',
  'page.parts.einstellungen': 'dx-settings-panel (4.11)|dx-robot-settings (4.7)|Diagnose|Version',

  // ── Flächen der Bento-Übersicht (pages.ts START_SLOTS) ──
  'slot.map': 'Live-Karte',
  'slot.automatik': 'Automatik',
  'slot.auftrag': 'Aktueller Auftrag',
  'slot.heute': 'Heute',
  'slot.planer': 'Planer',
  'slot.consumables': 'Verschleiß',
  'slot.station': 'Station',
  'slot.stats': 'Statistik',
  'slot.quickstart': 'Schnellstart – Räume auswählen',
  'slot.history': 'Letzte Läufe',

  // ── Kopfzeile (Shell) ──
  'topbar.back': 'Übersicht',
  'topbar.morning': 'Guten Morgen',
  'topbar.day': 'Guten Tag',
  'topbar.evening': 'Guten Abend',
  'topbar.away': '{name} ist unterwegs',
  'topbar.inStation': '{name} ist in der Station',
  'topbar.home': 'Zu Hause',
  'topbar.nobody': 'Niemand zu Hause',
  'topbar.present': '{names} anwesend',
  'topbar.allAway': 'alle unterwegs',
  'topbar.dnd': 'Nicht stören',
  'topbar.inDialog': 'Im Dialog: {hint}',

  // ── Roboter-Panel (dx-hero) ──
  'hero.status': 'Status',
  'hero.station': 'Station',
  'hero.washing': 'Mopp-Wäsche',
  'hero.drying': 'trocknet',
  'hero.docked': 'angedockt',
  'hero.charging': 'lädt',
  'hero.away': 'unterwegs',
  'hero.battery': 'Akku',
  'hero.mode': 'Modus',
  'hero.modeTitle': 'Reinigungsmodus',
  'hero.suction': 'Saugstufe',
  'hero.suctionTitle': 'Saugleistung',
  'hero.water': 'Wasser',
  'hero.waterTitle': 'Wassermenge',
  'hero.roomsTitle': 'Räume einstellen',
  'hero.ackTitle': 'Warnung quittieren',

  // ── Auftrag-Kachel (dx-auftrag) ──
  'auftrag.title': 'Aktueller Auftrag',
  'auftrag.rooms': 'Räume',
  'auftrag.noRooms': 'Räume –',
  'auftrag.first': 'Erster Raum',
  'auftrag.next': 'Nächster Raum',
  'auftrag.last': 'Letzter Raum',

  // ── Schnellstart (dx-quickstart) ──
  'quickstart.title': 'Schnellstart – Räume auswählen',
  'quickstart.multi': 'Mehrfachauswahl',
  'quickstart.selected': '{n} gewählt',
  'quickstart.all': 'Alles',
  'quickstart.hint': 'Räume antippen, dann „reinigen“ – {name} fährt mit den Roboter-Werten je Raum.',
  'quickstart.robotFallback': 'der Roboter',

  // ── Raumauswahl (shared/rooms, Karte, Schnellstart) ──
  'rooms.whole': 'Ganze Wohnung',
  'rooms.one': '{n} Raum',
  'rooms.many': '{n} Räume',
  'rooms.confirm': 'Jetzt reinigen: {list}?',
  'rooms.run': '{sel} reinigen',
  'rooms.clear': 'Auswahl aufheben',
  'rooms.started': 'Gestartet: {what}',
  'rooms.failed': 'Start fehlgeschlagen: {error}',

  // ── Karte (dx-map-card, dx-heidi-map, map-config) ──
  'map.title': 'Karte',
  'map.live': 'Live-Karte',
  'map.rooms': 'Räume',
  'map.zones': 'Sperrzonen',
  'map.history': 'Reinigungsverlauf',
  'map.toMap': 'Zur Karte',
  'map.open': 'Karte öffnen',
  'map.tapHint': 'antippen für Räume, Zone, Punkt, Sperrzonen',
  'map.capAway': 'unterwegs',
  'map.capRest': 'noch {rest}',
  'map.capStation': '{name} in der Station',
  'map.capLast': 'letzter Lauf {time}',
  'map.goto': 'Hinfahren',
  'map.all': 'Alles',
  'map.allConfirm': 'Ganze Wohnung reinigen?',
  'map.allStarted': 'ganze Wohnung',
  'map.hintHeidi': 'Räume in der Karte oder über die Kacheln antippen, dann „reinigen“',
  'map.hintPlain': 'Räume antippen, dann „reinigen“',
  'map.loadError': 'Karte konnte nicht geladen werden: {error}',
  'map.helpersMissing': 'loadCardHelpers fehlt',
  'map.alt': 'Karte',
  'map.noMapData': 'Datenkarte fehlt – {entity} in der Dreame-Integration aktivieren',
  'map.loading': 'Kartenpaket wird geladen …',
  'map.noCalib': 'Keine Kalibrierpunkte – Räume können nicht eingezeichnet werden',
  'mapmode.raeume': 'Räume',
  'mapmode.raeume.hint': 'Auswahl per Kachel oder Tipp in die Raumfläche',
  'mapmode.zone': 'Zone',
  'mapmode.zone.hint': 'Rechteck auf der Karte aufziehen (bis zu 5), dann ▶ in der Karte',
  'mapmode.punkt': 'Punkt',
  'mapmode.punkt.hint': 'Punkt auf der Karte antippen, dann ▶ in der Karte',
  'mapmode.goto': 'Hinfahren',
  'mapmode.goto.hint': 'Punkt auf der Karte antippen → der Roboter fährt hin und wartet',

  // ── Dialog (dx-dialog, Shell-Overlay) ──
  'dialog.overlay': 'Overlay „{kind}“',
  'dialog.placeholder': 'Platzhalter – der Inhalt dieses Dialogs entsteht mit seinem Baustein in Phase 4.',

  // ── Einrichtungsprüfung (domain/setup, dx-setup) ──
  'setup.title': 'Einrichtung',
  'setup.problem': 'Problem',
  'setup.problems': 'Probleme',
  'setup.hint': 'Hinweis',
  'setup.hints': 'Hinweise',
  'setup.there': 'Dort: {hint}',
  'setup.robot.label': 'Roboter',
  'setup.robot.ok': 'Roboter erkannt: {name}',
  'setup.robot.error': 'Kein Dreame-Roboter gefunden – Integration einrichten oder „robot:“ in der Kartenkonfiguration setzen',
  'setup.package.label': 'Paket',
  'setup.package.ok': 'Paket vollständig',
  'setup.package.error': '{n} Helfer des Pakets fehlen – Paket heidi.yaml und Automationen prüfen',
  'setup.entities.label': 'Roboter-Entitäten',
  'setup.entities.ok': 'Roboter-Entitäten vollständig',
  'setup.entities.warn': '{n} Entitäten des Roboters fehlen oder sind deaktiviert',
  'setup.mapdata.label': 'Datenkarte',
  'setup.mapdata.ok': 'Datenkarte aktiv',
  'setup.mapdata.warn': 'Datenkarte nicht aktiviert – Räume in der Heidi-Karte fehlen (Entität „Current Map Data“ in der Dreame-Integration einschalten)',
  'setup.customized.label': 'Angepasste Reinigung',
  'setup.customized.ok': 'Angepasste Reinigung an',
  'setup.customized.warn': 'Angepasste Reinigung ist aus – Raumwerte je Raum wirken nicht',
  'setup.roomtypes.label': 'Raumtypen',
  'setup.roomtypes.ok': 'Raumtypen passen',
  'setup.roomtypes.warn': '{rooms}: benutzerdefiniert, obwohl die App den Standardtyp kennt – für die Sprachsteuerung in der App „Raum umbenennen“ den Typ wählen',
  'setup.areas.label': 'Räume ↔ Bereiche',
  'setup.areas.ok': 'Alle Räume einem HA-Bereich zugeordnet',
  'setup.areas.one': 'Ein Raum ist',
  'setup.areas.many': '{n} Räume sind',
  'setup.areas.error': '{who} keinem HA-Bereich zugeordnet: {rooms}',
  'setup.areas.hint': 'Reinigung → Nach Bereich → Konfigurieren',
  'setup.repairs.label': 'Reparaturen',
  'setup.repairs.ok': 'Keine offenen Reparaturen',
  'setup.repairs.one': 'Reparatur',
  'setup.repairs.many': 'Reparaturen',
  'setup.repairs.error': '{n} offene {what} in HA ({list})',

  // ── Seite Reinigen (Shell) ──
  'reinigen.scenes': 'Dreame-App-Szenen',
  'reinigen.app': 'App',
  'reinigen.sceneConfirm': '„{name}“ starten?',
  'reinigen.chairs': 'Stühle am Boden',
  'reinigen.chairsSub': 'Setzt eine Sperrzone um den Esstisch',
  'reinigen.roomsTitle': 'Räume (Roboter-Werte)',
  'reinigen.roomsHint': 'Modus, Saugstufe, Wasser, Route und Wiederholungen je Raum, wie in der Dreame-App. Änderungen gelten sofort.',
  'reinigen.roomsOpen': 'Räume einstellen …',
  'scene.32.name': 'Eingang reinigen',
  'scene.32.sub': 'Flur · Saugen + Wischen · 2×',
  'scene.33.name': 'Bad Saugen/Wischen',
  'scene.33.sub': 'Bad · 1×',
  'scene.34.name': 'Wischen nach dem Saugen',
  'scene.34.sub': 'Ganze Wohnung · nur Wischen',

  // ── Platzhalter-Vorschauen der Shell (verschwinden mit den Modulen) ──
  'preview.entities': '{n} Entitäten verbunden',
  'preview.noStates': 'keine Zustandsdaten',
  'preview.map': 'Kartendarstellung: {karte} · Kalibrierung: {calib}',
  'preview.calibPoints': '{n} Punkte',
  'preview.automatik': 'Automatik: {status}',
  'preview.today': 'Heutiger Eintrag: {name}',
  'preview.prognose': 'Freies Fenster {window} · Rückkehr {back}',
  'preview.prognoseOff': 'Prognose aus',
  'preview.stats': '{runs} Läufe · {area} m² · {time} min',
  'preview.rooms': 'Räume: {list}',
  'preview.noRooms': 'keine (Karte fehlt)',
  'preview.entries': '{n} Einträge',
  'preview.stale': ' · letzter Stand',
  'preview.placeholder': 'Platzhalter – entsteht in Aufgabe {task}.',
  'preview.mapPage': 'Kartendarstellung: {karte} · Stühle am Boden: {chairs} · Kalibrierung: {calib}',
  'preview.plan': '{n} {name} · {state} · {rooms} Räume · {time}',
  'preview.learn': 'Lernwerte: {state}',
  'preview.history': '{entries} Einträge · {runs} Läufe · {area} m² · {time} min{stale}',
  'preview.prognosePage': '{state} · {days} Tage · frei {window} · Rückkehr {back}',
  'preview.settings': 'Karte {karte} · Diagnose: {missing} fehlend, {unavailable} unavailable von {total}',
  'preview.robot': 'Roboter: {list} · DND {start}–{end}',
  'preview.pageTitle': 'Seite „{page}“',
  'preview.parts': 'Hier entstehen',
  'preview.views': 'Sichten (Vorschau aus den Selektoren)',
  'preview.mockup': 'Platzhalter aus Aufgabe 3.3 – Optik nach Mockup',

  // ── Karte in HA (customCards, Konfiguration) ──
  'card.name': 'Heidi (dreame_x60)',
  'card.description': 'Heidi-Karte v2 – Übersicht und Unterseiten des Saugroboters',
  'card.noConfig': 'dreame-x60-panel: Konfiguration fehlt',

  // ── Status des Roboters (sensor.<gerät>_status, vacuum-Zustand) – wie v1 STATUS_DE ──
  'status.sleeping': 'schläft',
  'status.charging': 'lädt',
  'status.cleaning': 'reinigt',
  'status.sweeping': 'saugt',
  'status.mopping': 'wischt',
  'status.sweeping_and_mopping': 'saugt und wischt',
  'status.returning': 'fährt zur Station',
  'status.paused': 'pausiert',
  'status.idle': 'bereit',
  'status.docked': 'angedockt',
  'status.washing': 'Mopp-Wäsche',
  'status.drying': 'trocknet',
  'status.auto_emptying': 'saugt ab',
  'status.error': 'Fehler',
  'status.charging_completed': 'voll geladen',
  'status.segment_cleaning': 'reinigt Räume',
  'status.zone_cleaning': 'reinigt Zone',
  'status.spot_cleaning': 'reinigt Punkt',
  'status.cruising': 'fährt',

  // ── Auftragsart (sensor.<gerät>_task_status) – wie v1 TASK ──
  'task.room_cleaning': 'Reinigt Räume',
  'task.zone_cleaning': 'Reinigt Zone',
  'task.spot_cleaning': 'Reinigt Punkt',
  'task.cleaning': 'Reinigt',
  'task.cruising': 'Fährt',
  'task.mapping': 'Erstellt Karte',
  'task.fast_mapping': 'Erstellt Karte',
  'task.default': 'Reinigt',

  // ── Kopf (domain/status) ──
  'head.error': 'Fehler',
  'head.paused': 'Pausiert',
  'head.returning': 'Fährt zur Station',
  'button.pause': 'Pause',
  'button.stop': 'Stopp',
  'button.station': 'Station',
  'button.resume': 'Weiter',
  'button.start': 'Start',
  'button.locate': 'Orten',

  // ── Beschriftungen (domain/labels) ──
  'label.daily': 'Täglich',
  'label.manual': 'Manuell',
  'label.weekdays': 'Mo–Fr',
  'label.weekend': 'Sa + So',
  'label.allRooms': 'Alle',
  'label.noRooms': 'keine Räume',
  'label.min': '{n} min',
  'label.hmin': '{h} h {m} min',
  'label.lessMin': '< 1 min',
  'label.today': 'heute',
  'day.0': 'Mo',
  'day.1': 'Di',
  'day.2': 'Mi',
  'day.3': 'Do',
  'day.4': 'Fr',
  'day.5': 'Sa',
  'day.6': 'So',

  // ── Dauer & Akku (domain/estimate) ──
  'estimate.washBefore': 'Wäscht Mopp vor dem Start',
  'estimate.washBetween': 'Wäscht Mopp zwischendurch',
  'estimate.charge': 'Lädt {from} % → {to} %',
  'estimate.home': 'Fährt zur Station',
  'estimate.sourcePrognose': 'Prognose',
  'estimate.sourceUsual': 'übliche Rückkehr {time}',

  // ── Streifen (domain/strip) ──
  'strip.startpoint': 'Fährt zum Startpunkt',
  'strip.to': 'zu {rooms}',
  'strip.through': 'Fährt durch {room}',
  'strip.now': 'Jetzt: {room}',
  'strip.then': 'danach {rooms}',
  'strip.lastRoom': 'letzter Raum',

  // ── Optionen des Roboters über die v1-Zuordnung hinaus (ha/profile) ──
  'option.mopping_after_sweeping': 'Wischen nach Saugen',
  'option.quick': 'Schnell',
  'option.off': 'Aus',
  'option.on': 'An',

  // ── Raumnamen der Integration → deutsch (v1 ROOMS_DE) ──
  'room.Bathroom': 'Bad',
  'room.Primary Bedroom': 'Schlafzimmer',
  'room.WC': 'WC',
  'room.Corridor': 'Flur',
  'room.Study': 'Büro',
  'room.Kitchen': 'Küche',
  'room.Living Room': 'Wohnzimmer',
  'room.fallback': 'Raum {n}',

  // ── Raumtypen der Dreame-App (domain/rooms ROOM_TYPES, type 1..15) ──
  'roomtype.1': 'Wohnzimmer',
  'roomtype.2': 'Schlafzimmer',
  'roomtype.3': 'Arbeitszimmer',
  'roomtype.4': 'Küche',
  'roomtype.5': 'Esszimmer',
  'roomtype.6': 'Bad',
  'roomtype.7': 'Balkon',
  'roomtype.8': 'Flur',
  'roomtype.9': 'Allzweckraum',
  'roomtype.10': 'Garderobe',
  'roomtype.11': 'Salon',
  'roomtype.12': 'Büro',
  'roomtype.13': 'Fitnessbereich',
  'roomtype.14': 'Freizeitbereich',
  'roomtype.15': 'Nebenzimmer',

  // ── Einstellungen (ha/selectors readSettings) ──
  'settings.automatik': 'Automatik',
  'settings.planer': 'Planer anzeigen',
  'settings.prognose': 'Prognose',
  'settings.prognoseSub': 'Lernende Anwesenheit, eigene Seite',
  'settings.nina': 'Nina zählt für Anwesenheit',
  'settings.interval': 'Protokoll-Intervall',
  'settings.intervalSub': 'Wie oft die Anwesenheit gespeichert wird',
  'settings.resolution': 'Auflösung',
  'settings.resolutionSub': 'Rasterbreite der Heatmap und Prognose',
  'settings.weeks': 'Lernzeitraum',
  'settings.weeksSub': 'Ältere Daten werden verworfen',
  'settings.halflife': 'Gewichtung',
  'settings.halflifeSub': 'Halbwertszeit – so alt zählt ein Tag nur noch halb',
  'settings.minDays': 'Aktiv ab',
  'settings.minDaysSub': 'Erst dann nutzt die Automatik die Prognose',
  'settings.unitMin': ' min',
  'settings.unitWeeks': ' Wochen',
  'settings.unitDays': ' Tage',
  'settings.unitDaysFrom': ' Tagen',
  'settings.original': 'Original',
  'settings.german': 'Deutsch',

  // ── Prognose-Schalter (readPrognose) ──
  'prognose.deviation': 'Abweichung heute',
  'prognose.deviationSub': 'Urlaub, Feiertag',
  'prognose.include': '{name} einbeziehen',
  'prognose.gpsWlan': 'GPS + WLAN',
  'prognose.wlan': 'WLAN',

  // ── Station (readStation) ──
  'station.bag': 'Beutel',
  'station.fresh': 'Frisch',
  'station.dirty': 'Abwasser',
  'station.detergent': 'Mittel',
  'station.ok': 'OK',
  'station.check': 'Prüfen',
  'station.empty': 'Leer',
  'station.missing': 'Fehlt',
  'station.full': 'Voll',
  'station.autoEmpty': 'Absaugen',
  'station.mop': 'Mopp',
  'station.dry': 'Trocknen',
  'station.clean': 'Station',
  'station.cleanConfirm': 'Reinigung der Station starten?',

  // ── Verschleiß (readConsumables) ──
  'consumable.mainBrush': 'Hauptbürste',
  'consumable.sideBrush': 'Seitenbürste',
  'consumable.filter': 'Filter',
  'consumable.sensor': 'Sensoren',
  'consumable.wheel': 'Räder',

  // ── Roboter-Einstellungen (readRobotSettings) ──
  'robotsettings.carpet': 'Teppich',
  'robotsettings.waterTemp': 'Wassertemperatur',
  'robotsettings.drying': 'Trocknung',
  'robotsettings.autoEmpty': 'Absaugen',
  'robotsettings.selfClean': 'Mopp-Wäsche',
  'robotsettings.cleangenius': 'CleanGenius',
  'robotsettings.selfCleanArea': 'Mopp-Wäsche nach',
  'robotsettings.volume': 'Lautstärke',
  'robotsettings.unitM2': ' m²',
  'robotsettings.unitPct': ' %',

  // ── Diagnose, Automatik ──
  'diag.robot': 'Roboter ({name})',
  'diag.robotUnknown': 'nicht erkannt',
  'diag.package': 'Paket (Helfer, Sensoren)',
  'automatik.never': 'noch nie',

  // ── Hinweise und Fehler des Roboters (sensor.<gerät>_error): Kurztext (höchstens zwei Wörter) im Chip ──
  // Quelle: docs/dreame_x60/ENTITAETEN.md „Warnungen und Fehler“ (Integration dreame-vacuum 2.0.0b25); dazu die
  // v1-Schlüssel, die es dort nicht gibt (brush_stuck …), damit die v1-Vektoren gleich bleiben.
  'error.drop': 'Räder frei',
  'error.cliff': 'Absturzsensor',
  'error.bumper': 'Stoßsensor klemmt',
  'error.gesture': 'Steht schräg',
  'error.bumper_repeat': 'Stoßsensor klemmt',
  'error.drop_repeat': 'Räder frei',
  'error.optical_flow': 'Sensorfehler',
  'error.no_box': 'Staubbox fehlt',
  'error.no_tank_box': 'Tank fehlt',
  'error.water_box_empty': 'Wassertank leer',
  'error.box_full': 'Filter verstopft',
  'error.brush': 'Hauptbürste verwickelt',
  'error.side_brush': 'Seitenbürste verwickelt',
  'error.fan': 'Filter verstopft',
  'error.left_wheel_motor': 'Rad blockiert',
  'error.right_wheel_motor': 'Rad blockiert',
  'error.turn_suffocate': 'Steckt fest',
  'error.forward_suffocate': 'Steckt fest',
  'error.charger_get': 'Station fehlt',
  'error.battery_low': 'Akku schwach',
  'error.charge_fault': 'Ladefehler',
  'error.battery_percentage': 'Akkufehler',
  'error.heart': 'Interner Fehler',
  'error.camera_occlusion': 'Sensor verdeckt',
  'error.move': 'Sensorfehler',
  'error.flow_shielding': 'Sensor verdeckt',
  'error.infrared_shielding': 'Sensor verdeckt',
  'error.charge_no_electric': 'Station stromlos',
  'error.battery_fault': 'Akkutemperatur',
  'error.fan_speed_error': 'Lüfterfehler',
  'error.left_wheell_speed': 'Rad blockiert',
  'error.right_wheell_speed': 'Rad blockiert',
  'error.bmi055_acce': 'Sensorfehler',
  'error.bmi055_gyro': 'Gyroskopfehler',
  'error.xv7001': 'Gyroskopfehler',
  'error.left_magnet': 'Magnetsensor',
  'error.right_magnet': 'Magnetsensor',
  'error.flow_error': 'Sensorfehler',
  'error.infrared_fault': 'Infrarotfehler',
  'error.camera_fault': 'Kamerafehler',
  'error.strong_magnet': 'Magnetfeld',
  'error.water_pump': 'Wasserpumpe',
  'error.rtc': 'Uhrfehler',
  'error.auto_key_trig': 'Interner Fehler',
  'error.p3v3': 'Interner Fehler',
  'error.camera_idle': 'Interner Fehler',
  'error.blocked': 'Weg blockiert',
  'error.lds_error': 'Laserfehler',
  'error.lds_bumper': 'Laserturm klemmt',
  'error.filter_blocked': 'Filter verstopft',
  'error.edge': 'Kantensensor',
  'error.carpet': 'Teppich erkannt',
  'error.laser': 'Sensor gestört',
  'error.ultrasonic': 'Ultraschallsensor',
  'error.no_go_zone': 'In Sperrzone',
  'error.route': 'Ziel unerreichbar',
  'error.restricted': 'In Sperrzone',
  'error.remove_mop': 'Mopp abnehmen',
  'error.mop_removed': 'Mopp abgefallen',
  'error.mop_pad_stop_rotate': 'Mopp blockiert',
  'error.mop_install_failed': 'Mopp anbringen',
  'error.low_battery_turn_off': 'Akku leer',
  'error.dirty_tank_not_installed': 'Abwassertank fehlt',
  'error.robot_in_hidden_room': 'Bereich ausgeblendet',
  'error.lds_failed_to_lift': 'Laserturm klemmt',
  'error.robot_stuck': 'Steckt fest',
  'error.slippery_floor': 'Rutschiger Boden',
  'error.unknown': 'Unbekannte Meldung',
  'error.check_mop_install': 'Mopp prüfen',
  'error.dirty_water_tank_full': 'Abwasser voll',
  'error.retractable_leg_stuck': 'Beine verwickelt',
  'error.internal_error': 'Interner Fehler',
  'error.robot_stuck_on_tables': 'Steckt fest',
  'error.robot_stuck_on_passage': 'Steckt fest',
  'error.robot_stuck_on_threshold': 'Steckt fest',
  'error.robot_stuck_on_low_lying_area': 'Steckt fest',
  'error.robot_stuck_on_ramp': 'Rampe erkannt',
  'error.robot_stuck_on_obstacle': 'Hindernis',
  'error.robot_stuck_on_pet': 'Person/Tier',
  'error.robot_stuck_on_slippery_surface': 'Rutscht',
  'error.robot_stuck_on_carpet': 'Steckt fest',
  'error.bin_full': 'Staubbeutel voll',
  'error.bin_open': 'Station offen',
  'error.water_tank': 'Frischwassertank fehlt',
  'error.dirty_water_tank': 'Abwasser voll',
  'error.water_tank_dry': 'Frischwasser leer',
  'error.dirty_water_tank_blocked': 'Abwassertank verstopft',
  'error.dirty_water_tank_pump': 'Abwasserpumpe',
  'error.mop_pad': 'Waschbrett fehlt',
  'error.wet_mop_pad': 'Waschbrett reinigen',
  'error.clean_mop_pad': 'Mopp reinigen',
  'error.clean_tank_level': 'Frischwasser knapp',
  'error.station_disconnected': 'Station getrennt',
  'error.dirty_tank_level': 'Abwasser voll',
  'error.washboard_level': 'Waschbrett reinigen',
  'error.no_mop_in_station': 'Mopp fehlt',
  'error.dust_bag_full': 'Staubbeutel voll',
  'error.self_test_failed': 'Selbsttest fehlgeschlagen',
  'error.washboard_not_working': 'Waschbrett defekt',
  'error.drainage_failed': 'Abpumpen gestört',
  'error.mop_not_detected': 'Mopp fehlt',
  'error.mop_holder_error': 'Mopphalter falsch',
  'error.dock_error': 'Stationsfehler',
  'error.wash_failed': 'Wäsche fehlgeschlagen',
  'error.robot_stuck_on_curtain': 'Steckt fest',
  'error.edge_mop_stop_rotate': 'Kantenmopp blockiert',
  'error.edge_mop_detached': 'Kantenmopp abgefallen',
  'error.chassis_lift_malfunction': 'Fahrwerk defekt',
  'error.mop_cover_error': 'Moppabdeckung',
  'error.roller_mop_error': 'Rollenmopp gestört',
  'error.robotic_arm_stopped': 'Arm gestoppt',
  'error.onboard_water_tank_empty': 'Wassertank leer',
  'error.onboard_dirty_water_tank_full': 'Abwasserbox voll',
  'error.mop_not_installed': 'Mopp fehlt',
  'error.fluffing_roller_error': 'Rolle gestört',
  'error.blocked_by_obstacle': 'Hindernis',
  'error.return_to_charge_failed': 'Rückkehr fehlgeschlagen',
  'error.drainage_outlet_filter': 'Abwasserfilter verstopft',
  'error.main_wheels_error': 'Radfehler',
  // v1-Schlüssel ohne Gegenstück in dieser Integrationsversion (Vektoren)
  'error.clean_water_tank_empty': 'Frischwasser leer',
  'error.dust_box_missing': 'Staubbox fehlt',
  'error.wheels_stuck': 'Rad blockiert',
  'error.brush_stuck': 'Bürste blockiert',
  'error.low_battery': 'Akku leer',
  'error.detergent_empty': 'Reinigungsmittel leer',
  'error.water_tank_missing': 'Wassertank fehlt',
  'error.clean_water_tank_missing': 'Frischwassertank fehlt',
  'error.dirty_water_tank_missing': 'Abwassertank fehlt',

  // ── Langtext je Zustandswert (Tooltip / Detail, PD-015) – Bedeutung aus ENTITAETEN.md ──
  'errorLong.drop': 'Räder hängen in der Luft – Roboter neu aufsetzen und neu starten.',
  'errorLong.cliff': 'Absturzsensor gestört – Sensor abwischen und nicht direkt an der Treppe starten.',
  'errorLong.bumper': 'Stoßsensor klemmt – reinigen und vorsichtig antippen.',
  'errorLong.gesture': 'Roboter steht schräg – auf eine ebene Fläche setzen und neu starten.',
  'errorLong.bumper_repeat': 'Stoßsensor klemmt wiederholt – reinigen und vorsichtig antippen.',
  'errorLong.drop_repeat': 'Räder hängen wiederholt in der Luft – Roboter neu aufsetzen.',
  'errorLong.optical_flow': 'Optischer Flusssensor gestört – Roboter neu starten.',
  'errorLong.no_box': 'Staubbox nicht eingesetzt – Staubbox und Filter einsetzen.',
  'errorLong.no_tank_box': 'Wassertank nicht eingesetzt – Tank einsetzen.',
  'errorLong.water_box_empty': 'Wassertank leer – bitte auffüllen.',
  'errorLong.box_full': 'Filter feucht oder verstopft – prüfen, ob der Filter trocken ist oder gereinigt werden muss.',
  'errorLong.brush': 'Hauptbürste verwickelt – Bürste herausnehmen, Borsten und Lager reinigen.',
  'errorLong.side_brush': 'Seitenbürste verwickelt – abnehmen und reinigen.',
  'errorLong.fan': 'Filter feucht oder verstopft – prüfen, ob der Filter trocken ist oder gereinigt werden muss.',
  'errorLong.left_wheel_motor': 'Roboter steckt fest oder das linke Rad ist blockiert – Räder prüfen und an neuer Stelle starten.',
  'errorLong.right_wheel_motor': 'Roboter steckt fest oder das rechte Rad ist blockiert – Räder prüfen und an neuer Stelle starten.',
  'errorLong.turn_suffocate': 'Roboter steckt fest und kann nicht drehen – Umgebung freiräumen.',
  'errorLong.forward_suffocate': 'Roboter steckt fest und kann nicht vorwärts – Umgebung freiräumen.',
  'errorLong.charger_get': 'Station nicht gefunden – prüfen, ob das Stromkabel richtig steckt.',
  'errorLong.battery_low': 'Akku schwach – bitte laden.',
  'errorLong.charge_fault': 'Ladefehler – Ladekontakte von Roboter und Station mit einem trockenen Tuch abwischen.',
  'errorLong.battery_percentage': 'Fehler beim Akkustand.',
  'errorLong.heart': 'Interner Fehler – Roboter neu starten.',
  'errorLong.camera_occlusion': 'Sensor der Bildpositionierung verdeckt – bitte reinigen.',
  'errorLong.move': 'Bewegungssensor gestört – Roboter neu starten.',
  'errorLong.flow_shielding': 'Optischer Sensor verdeckt – abwischen und neu starten.',
  'errorLong.infrared_shielding': 'Infrarotsensor verdeckt – Roboter neu starten.',
  'errorLong.charge_no_electric': 'Ladestation ohne Strom – prüfen, ob das Stromkabel richtig steckt.',
  'errorLong.battery_fault': 'Akkutemperatur außerhalb des Bereichs – warten, bis sie wieder normal ist.',
  'errorLong.fan_speed_error': 'Lüfterdrehzahl-Sensor gestört – Roboter neu starten.',
  'errorLong.left_wheell_speed': 'Linkes Rad blockiert – Räder prüfen und an neuer Stelle starten.',
  'errorLong.right_wheell_speed': 'Rechtes Rad blockiert – Räder prüfen und an neuer Stelle starten.',
  'errorLong.bmi055_acce': 'Beschleunigungssensor gestört – Roboter neu starten.',
  'errorLong.bmi055_gyro': 'Gyroskop gestört – Roboter neu starten.',
  'errorLong.xv7001': 'Gyroskop gestört – Roboter neu starten.',
  'errorLong.left_magnet': 'Magnetsensor links gestört – Roboter neu starten.',
  'errorLong.right_magnet': 'Magnetsensor rechts gestört – Roboter neu starten.',
  'errorLong.flow_error': 'Flusssensor gestört – Roboter neu starten.',
  'errorLong.infrared_fault': 'Infrarot gestört – Roboter neu starten.',
  'errorLong.camera_fault': 'Kamera gestört – Roboter neu starten.',
  'errorLong.strong_magnet': 'Starkes Magnetfeld erkannt – nicht direkt an der virtuellen Wand starten.',
  'errorLong.water_pump': 'Wasserpumpe gestört – Roboter neu starten.',
  'errorLong.rtc': 'Uhr (RTC) gestört – Roboter neu starten.',
  'errorLong.auto_key_trig': 'Interner Fehler – Roboter neu starten.',
  'errorLong.p3v3': 'Interner Fehler – Roboter neu starten.',
  'errorLong.camera_idle': 'Interner Fehler – Roboter neu starten.',
  'errorLong.blocked': 'Reinigungsweg blockiert oder Roboter steckt fest – Türen öffnen, Hindernisse oder Sperrzone im Weg räumen.',
  'errorLong.lds_error': 'Laser-Abstandssensor gestört – auf Fremdkörper prüfen.',
  'errorLong.lds_bumper': 'Stoßsensor des Laserturms klemmt – prüfen.',
  'errorLong.filter_blocked': 'Filter feucht oder verstopft – prüfen, ob der Filter trocken ist oder gereinigt werden muss.',
  'errorLong.edge': 'Kantensensor gestört – prüfen und reinigen.',
  'errorLong.carpet': 'Teppich beim Wischen erkannt – Roboter an eine andere Stelle setzen und neu starten.',
  'errorLong.laser': '3D-Hindernissensor gestört – Sensor reinigen.',
  'errorLong.ultrasonic': 'Ultraschallsensor gestört – Roboter neu starten.',
  'errorLong.no_go_zone': 'Sperrzone oder virtuelle Wand erkannt – Roboter aus dem Bereich setzen und neu starten.',
  'errorLong.route': 'Zielbereich nicht erreichbar – Türen öffnen, Hindernisse räumen oder Sperrzone im Weg löschen.',
  'errorLong.restricted': 'Roboter steht in einer Sperrzone – aus dem Bereich setzen.',
  'errorLong.remove_mop': 'Wischen fertig – Mopp abnehmen und reinigen.',
  'errorLong.mop_removed': 'Mopp-Pad während der Reinigung abgefallen – vor dem Weiterfahren anbringen.',
  'errorLong.mop_pad_stop_rotate': 'Mopp-Pad dreht sich nicht – bitte prüfen.',
  'errorLong.mop_install_failed': 'Mopp-Pad automatisch anbringen fehlgeschlagen – von Hand anbringen.',
  'errorLong.low_battery_turn_off': 'Akku leer – Roboter schaltet gleich ab.',
  'errorLong.dirty_tank_not_installed': 'Schmutzwassertank im Roboter fehlt – richtig einsetzen und Lauf starten.',
  'errorLong.robot_in_hidden_room': 'Roboter steht in einem ausgeblendeten Bereich – an passende Stelle setzen und neu versuchen.',
  'errorLong.lds_failed_to_lift': 'Laserturm lässt sich nicht anheben – Umgebung freiräumen und Roboter in einen offenen Bereich setzen.',
  'errorLong.robot_stuck': 'Roboter steckt fest oder kann sich nicht positionieren – in einen offenen Bereich setzen und fortsetzen.',
  'errorLong.slippery_floor': 'Rutschiger Boden – Roboter kommt nicht über das Hindernis; warten oder Wasser um den Roboter aufwischen.',
  'errorLong.unknown': 'Unbekannte Meldung des Roboters.',
  'errorLong.check_mop_install': 'Mopp-Halterung prüfen – ist der Mopp richtig eingesetzt?',
  'errorLong.dirty_water_tank_full': 'Schmutzwassertank im Roboter zu voll oder verschmutzt – leeren und reinigen.',
  'errorLong.retractable_leg_stuck': 'Ausfahrbare Beine verwickelt – prüfen.',
  'errorLong.internal_error': 'Interner Fehler – Roboter neu starten.',
  'errorLong.robot_stuck_on_tables': 'Steckt zwischen Tischen und Stühlen fest – in einen offenen Bereich setzen und neu starten.',
  'errorLong.robot_stuck_on_passage': 'Steckt in einem engen Durchgang fest – in einen offenen Bereich setzen; Durchgang als Sperrzone eintragen.',
  'errorLong.robot_stuck_on_threshold': 'Steckt an einer Stufe oder Schwelle fest – in einen offenen Bereich setzen; Schwelle als Sperrzone eintragen.',
  'errorLong.robot_stuck_on_low_lying_area': 'Steckt unter einem niedrigen Möbel fest – in einen offenen Bereich setzen; Bereich als Sperrzone eintragen.',
  'errorLong.robot_stuck_on_ramp': 'Absturzgefährdete Rampe auf dem Weg erkannt – passierbare Schwellen einstellen, wenn keine Rampe da ist.',
  'errorLong.robot_stuck_on_obstacle': 'Hindernis auf dem Weg – räumen und Lauf neu starten.',
  'errorLong.robot_stuck_on_pet': 'Person oder Haustier auf dem Weg – beim Neustart den Weg freihalten.',
  'errorLong.robot_stuck_on_slippery_surface': 'Steckt fest wegen Rutschens – Hauptreifen reinigen.',
  'errorLong.robot_stuck_on_carpet': 'Rutscht auf dem Teppich – vom Teppich setzen und neu starten; Teppich als Sperrzone eintragen.',
  'errorLong.bin_full': 'Staubbeutel voll oder Luftkanal verstopft – prüfen.',
  'errorLong.bin_open': 'Deckel der Absaugstation offen oder Staubbeutel fehlt – prüfen.',
  'errorLong.water_tank': 'Frischwassertank nicht eingesetzt – einsetzen.',
  'errorLong.dirty_water_tank': 'Schmutzwassertank voll oder nicht eingesetzt – prüfen.',
  'errorLong.water_tank_dry': 'Zu wenig Wasser im Frischwassertank – nachfüllen, sonst fährt der Roboter nicht zur Mopp-Wäsche.',
  'errorLong.dirty_water_tank_blocked': 'Schmutzwassertank verstopft – Roboter neu starten.',
  'errorLong.dirty_water_tank_pump': 'Pumpe des Schmutzwassertanks gestört – Roboter neu starten.',
  'errorLong.mop_pad': 'Waschbrett nicht richtig eingesetzt – Roboter kann nicht zur Waschstation; Waschbrett und Verschlüsse prüfen.',
  'errorLong.wet_mop_pad': 'Wasserstand im Waschbrett abnormal – Waschbrett rechtzeitig reinigen, sonst droht Verstopfung.',
  'errorLong.clean_mop_pad': 'Reinigung fertig – Waschbrett des Mopps rechtzeitig reinigen, sonst Flecken und Geruch.',
  'errorLong.clean_tank_level': 'Frischwassertank bald leer – prüfen und nachfüllen.',
  'errorLong.station_disconnected': 'Station ohne Strom – Stromversorgung und Netzstecker der Station prüfen.',
  'errorLong.dirty_tank_level': 'Schmutzwassertank zu voll – prüfen und leeren.',
  'errorLong.washboard_level': 'Wasserstand im Waschbrett zu hoch – Schmutzwassertank und Waschbrett reinigen.',
  'errorLong.no_mop_in_station': 'Mopp nicht in der Station – in die Station legen oder am Roboter anbringen.',
  'errorLong.dust_bag_full': 'Staubbeutel prüfen und wechseln. Absaugöffnungen von Staubbox und Station regelmäßig reinigen.',
  'errorLong.self_test_failed': 'Selbsttest fehlgeschlagen – kein Wasser im Frischwassertank des Wassermoduls.',
  'errorLong.washboard_not_working': 'Waschbrett arbeitet nicht – auf Verwicklung prüfen und reinigen.',
  'errorLong.drainage_failed': 'Abpumpen des Schmutzwassers gestört – Kundendienst kontaktieren.',
  'errorLong.mop_not_detected': 'Mopp nicht erkannt – anbringen und Lauf fortsetzen.',
  'errorLong.mop_holder_error': 'Mopp-Halter in der Station falsch – Anzahl und Lage prüfen.',
  'errorLong.dock_error': 'Stationsfehler – Klappe ganz schließen und Mopps richtig einlegen.',
  'errorLong.wash_failed': 'Mopp-Wäsche fehlgeschlagen – Station prüfen, Mopp von Hand anbringen.',
  'errorLong.robot_stuck_on_curtain': 'Steckt im Vorhang fest – Roboter wegsetzen und neu starten; Bereich als Sperrzone eintragen.',
  'errorLong.edge_mop_stop_rotate': 'Kanten-Mopp dreht sich nicht – prüfen, ob der Roboter auf Teppich steht.',
  'errorLong.edge_mop_detached': 'Kanten-Mopp abgefallen – vor dem Weiterfahren anbringen.',
  'errorLong.chassis_lift_malfunction': 'Fahrwerkshub defekt – Lauf neu starten; bleibt es, Kundendienst kontaktieren.',
  'errorLong.mop_cover_error': 'Mopp-Abdeckung gestört – Rollen-Mopp und Abdeckung reinigen, Roboter flach hinstellen und fortsetzen.',
  'errorLong.roller_mop_error': 'Rollen-Mopp gestört – Rollen-Mopp und Abdeckung reinigen, Roboter flach hinstellen und fortsetzen.',
  'errorLong.robotic_arm_stopped': 'Roboterarm gestoppt – Stationsknopf und Arm-Knopf gedrückt halten, um zurückzusetzen.',
  'errorLong.onboard_water_tank_empty': 'Wassertank im Roboter leer – bitte nachfüllen.',
  'errorLong.onboard_dirty_water_tank_full': 'Schmutzwasserbox im Roboter voll – herausnehmen und leeren, oder zur Mopp-Wäsche in die Station.',
  'errorLong.mop_not_installed': 'Mopp nicht eingesetzt – vor dem Start prüfen, ob der Rollen-Mopp richtig sitzt.',
  'errorLong.fluffing_roller_error': 'Aufplusterrolle gestört – Roboter umdrehen, Rolle entriegeln und reinigen.',
  'errorLong.blocked_by_obstacle': 'Von einem Hindernis blockiert – vor dem Roboter räumen.',
  'errorLong.return_to_charge_failed': 'Rückkehr zur Station fehlgeschlagen – Rampe und Stromversorgung der Station prüfen.',
  'errorLong.drainage_outlet_filter': 'Abwasserfilter des Roboters verstopft – reinigen, damit das Wasser fließt.',
  'errorLong.main_wheels_error': 'Hauptrad-Fehler – Reinigung pausiert; am Roboter oder in der App fortsetzen.',
  'errorLong.clean_water_tank_empty': 'Frischwassertank leer – nachfüllen.',
  'errorLong.dust_box_missing': 'Staubbox nicht eingesetzt.',
  'errorLong.wheels_stuck': 'Rad blockiert – Räder prüfen.',
  'errorLong.brush_stuck': 'Bürste blockiert – Bürste prüfen und reinigen.',
  'errorLong.low_battery': 'Akku leer – bitte laden.',
  'errorLong.detergent_empty': 'Reinigungsmittel leer – nachfüllen.',
  'errorLong.water_tank_missing': 'Wassertank nicht eingesetzt.',
  'errorLong.clean_water_tank_missing': 'Frischwassertank nicht eingesetzt.',
  'errorLong.dirty_water_tank_missing': 'Abwassertank nicht eingesetzt.',
} as const;
