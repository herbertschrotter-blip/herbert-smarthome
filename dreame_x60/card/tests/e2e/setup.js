...
// Knopf in der Kopfzeile (nur bei Befund) → Dialog „Einrichtung“ mit dx-setup
const setup = async (page) => {
  const item = await page.evaluate(() => { const b = document.querySelector('dreame-x60-panel').shadowRoot.querySelector('.topbar .mi.setup'); if (!b) return null; const r = { cls: b.className, sum: b.querySelector('b').textContent }; b.click(); return r; });
  if (!item) return { visible: false };
  await page.waitForTimeout(150);
  return page.evaluate((item) => { const d = document.querySelector('dreame-x60-panel').shadowRoot.querySelector('dx-dialog[data-kind="setup"]'); const el = d && d.querySelector('dx-setup'); const bar = el && el.shadowRoot.querySelector('.bar'); return { visible: !!bar, dialog: !!d, cls: item.cls.includes('error') ? 'bar error' : 'bar warn', icons: bar ? [...bar.querySelectorAll('.icons .ic')].map((i) => i.dataset.check + ':' + i.className.replace('ic ', '')) : [], problems: bar ? [...bar.querySelectorAll('[data-problem]')].map((p) => p.dataset.problem + ' | ' + p.querySelector('.tx').textContent.replace(/\s+/g, ' ').trim()) : [], sum: item.sum }; }, item);
};
...
// ── alles eingerichtet → keine Leiste ──
{
  ...
  H.check('alles eingerichtet: kein Knopf in der Kopfzeile', !s.visible, s);
...
// ── Schalter aus, zwei Räume ohne Bereich, eine Reparatur ──
{
  ...
  await tick(page);
  const s = await setup(page);
  H.check('Knopf in der Kopfzeile rot, Dialog „Einrichtung“ offen', s.visible && s.dialog && s.cls === 'bar error', s);
  ...
  await page.evaluate(() => document.querySelector('dreame-x60-panel').shadowRoot.querySelector('dx-dialog[data-kind="setup"] dx-setup').shadowRoot.querySelector('.icons [data-check="areas"]').click());
  await page.evaluate(() => document.querySelector('dreame-x60-panel').shadowRoot.querySelector('dx-dialog[data-kind="setup"] dx-setup').shadowRoot.querySelector('[data-go="customized"]').click());
  await tick(page, 80);
  ...
  await page.evaluate(() => document.querySelector('dreame-x60-panel').shadowRoot.querySelector('dx-dialog[data-kind="setup"] dx-setup').shadowRoot.querySelector('[data-go="repairs"]').click());
  await tick(page, 80);
...
// ── ohne WS (alte HA, Tests): Bereichs- und Reparaturprüfung entfallen, Rest läuft ──
{
  ...
  await page.evaluate(() => document.querySelector('dreame-x60-panel').shadowRoot.querySelector('dx-dialog[data-kind="setup"] dx-setup').shadowRoot.querySelector('[data-go="mapdata"]').click());
  await tick(page, 80);
...
// ── Raumtypen: Abzug unverändert (alle benutzerdefiniert) → Hinweis, WC nicht dabei ──
{
  ...
  await listenMoreInfo(page);
  await page.evaluate(() => document.querySelector('dreame-x60-panel').shadowRoot.querySelector('dx-dialog[data-kind="setup"] dx-setup').shadowRoot.querySelector('[data-go="roomtypes"]').click());
  await tick(page, 80);
...
// ── Paket fehlt → Sprung zur Seite Einstellungen ──
{
  ...
  await page.evaluate(() => document.querySelector('dreame-x60-panel').shadowRoot.querySelector('dx-dialog[data-kind="setup"] dx-setup').shadowRoot.querySelector('[data-go="package"]').click());
  await tick(page);
