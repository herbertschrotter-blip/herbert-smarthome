...
.er { display: grid; grid-template-columns: 26px 1fr auto; align-items: center; gap: 8px; padding: 6px 8px; border-radius: 10px; background: var(--surface-2); font-size: 12px; }
.estline + .estline { margin-left: 4px; }
...
class HeidiPanel extends HTMLElement {
  ...
  constructor() {
    ...
    this._est = null;         // Untermenü Dauer & Akku: { n, cmp (Vergleichslinie), back }
    this._view = "main";
    this._sig = "";
    this._mapEls = {};
    this._editing = null;
  ...
  _estLine(n) {
    ...
    const rest = this._restMin();
    const line = (est, label) => { const fits = est.total <= rest.min; return `<button class="estline ${fits ? "acc" : "bad"}" data-act="est" data-plan="${n}" title="Dauer & Akku">${ic("mdi:timer-outline")}${label}ca. ${this._fmtMin(est.total)} · ${fits ? "reicht" : "reicht nicht"}${est.charges ? ic("mdi:power-plug") : ""}</button>`; };
    return line(e, "") + (p.schnell ? line(this._estimate(p, "schnell"), "Schnell: ") : "");
  }
  _estHtml() {
    ...
    if (!lern) return `<div class="scrim" data-act="${close}"></div><div class="modal"><div class="box ed" role="dialog" style="width:min(600px,100%)">${head}<div class="hint">Noch keine Lernwerte – das Laufprotokoll sammelt sie ab dem ersten Lauf.</div></div></div>`;
    const e = this._estimate(p), rest = this._restMin(), now = new Date(), tm = (d) => d.toLocaleTimeString("de-AT", { hour: "2-digit", minute: "2-digit" });
    const fits = e.total <= rest.min;
    const sum = `<div class="esum ${fits ? "ok" : "bad"}"><div><b>${this._fmtMin(e.total)}</b> · fertig um ${tm(new Date(now.getTime() + e.total * 60000))} bei Start jetzt</div>
      <div>${rest.min > 0 ? `Rückkehr in ${this._fmtMin(rest.min)} (${esc(rest.quelle)}) → <b>${fits ? "reicht" : "reicht nicht"}</b>` : "Rückkehrzeit vorbei"}${e.charges ? ` · ${e.charges}× Nachladen` : ""} · Akku ${Math.round(e.batt0)} % → ${Math.round(e.battEnd)} %${e.unlearned ? " · teils geschätzt" : ""}</div></div>`;
    ...
    const rows = e.steps.map((s) => s.typ === "raum"
      ? `<div class="er"><span class="ei">${ic(ROOMS.find((r) => r.id === s.id)?.icon || "mdi:floor-plan")}</span><span class="et"><b>${esc(s.text)}</b><small>${esc(s.sub)} · ${Math.round(s.area)} m²${s.gelernt ? "" : " · geschätzt"}</small></span><span class="em">${this._fmtMin(s.min)}</span></div>`
      : `<div class="er ${s.typ}"><span class="ei">${ic(icon[s.typ])}</span><span class="et">${esc(s.text)}</span><span class="em">${this._fmtMin(s.min)}</span></div>`).join("");
    // Diagramm: eine Linie (Einstellung des Eintrags), optional Vergleich mit anderer Saugstufe
    const cmpSaug = p.saug === "Standard" ? "Turbo" : "Standard";
    const lines = [{ e, own: true, gelernt: !e.unlearned, col: "var(--accent)" }];
    if (st.cmp) lines.push({ e: this._estimate(p, "normal", { saug: cmpSaug }), own: false, gelernt: !!lern.raten?.[p.modus + "/" + cmpSaug], col: "var(--blue)" });
    const maxT = Math.max(60, rest.min > 0 ? rest.min * 1.1 : 0, ...lines.map((l) => l.e.total));
    const Wd = 600, Ht = 190, px = 44, py = 14, X = (m) => px + (m / maxT) * (Wd - px - 10), Y = (b) => py + (1 - b / 100) * (Ht - py - 26);
    const poly = lines.map((l) => { let pts = [[0, l.e.batt0]], t = 0; l.e.steps.forEach((s) => { t += s.min; pts.push([t, s.batt]); }); return `<polyline points="${pts.map(([m, b]) => `${X(m).toFixed(1)},${Y(b).toFixed(1)}`).join(" ")}" fill="none" stroke="${l.col}" stroke-width="${l.own ? 3 : 2}" ${l.gelernt ? "" : 'stroke-dasharray="5 4"'} stroke-linejoin="round"/><text x="${X(t).toFixed(1)}" y="${(Y(l.e.battEnd) - 6).toFixed(1)}" fill="${l.col}" font-size="10" text-anchor="end">${tm(new Date(now.getTime() + t * 60000))}</text>`; }).join("");
    ...
    const svg = `<svg viewBox="0 0 ${Wd} ${Ht}" class="estsvg">${grid}${ret}${poly}${xt.join("")}</svg>`;
    const legend = `<div class="chips"><span class="chip on">${esc(p.modus)} · ${esc(p.saug)}</span><button class="chip ${st.cmp ? "on" : ""}" data-act="est-cmp" style="color:${st.cmp ? "var(--blue)" : ""}">${ic("mdi:compare-horizontal")}Vergleich mit ${esc(cmpSaug)}</button>${e.unlearned || (st.cmp && !lines[1].gelernt) ? `<span class="hint">gestrichelt = geschätzt</span>` : ""}</div>`;
    return `<div class="scrim" data-act="${close}"></div><div class="modal"><div class="box ed" role="dialog" style="width:min(600px,100%)">${head}${sum}
      <div class="sec"><div class="lab">Ablauf in Roboter-Reihenfolge</div><div class="elist">${rows}</div></div>
      <div class="sec"><div class="lab">Akku über den Lauf</div>${legend}${svg}</div>
      <div class="foot"><button class="btn primary" data-act="${close}">${st.back === "editor" ? "Zurück zum Eintrag" : "Fertig"}</button></div></div></div>`;
  }
  // Lernwerte-Tabelle (Zahnrad-Panel)
  _lernHtml() {
    const lern = this._lern(); if (!lern) return "";
    const rows = Object.entries(lern.raten || {}).filter(([, r]) => r.laeufe > 0).map(([k, r]) => `<div class="lr"><span>${esc(k.replace("/", " · "))}</span><span>${r.min_pro_m2.toFixed(2)} min/m²</span><span>${r.pct_pro_min.toFixed(2)} %/min</span><span>${r.laeufe} ${r.laeufe === 1 ? "Lauf" : "Läufe"}</span></div>`).join("");
    ...
    return `<div><h3>Lernwerte <span class="d" style="display:inline;font-weight:400">· ${lern.laeufe_gesamt ?? "?"} Läufe</span></h3><div class="lern">${rows || `<div class="hint">noch keine</div>`}${lade}</div></div>`;
  }
  ...
  async _onClick(e) {
    ...
    if (t.dataset.act === "est") { const n = parseInt(t.dataset.plan); this._est = { n, cmp: false, back: this._editing === n ? "editor" : null }; this._renderOverlay(); return; }
    if (t.dataset.act === "est-cmp") { this._est.cmp = !this._est.cmp; this._renderOverlay(); return; }
